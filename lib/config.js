...
function resolveChoice(value, allowed, fallback, name) {
  ...
  if (!allowed.includes(normalizedFallback)) {
    throw new Error(`[config] Invalid default ${name}=${fallback}`);
  }
  const normalized = String(value || normalizedFallback).trim().toLowerCase();
  if (allowed.includes(normalized)) return normalized;
  console.warn(`[config] Invalid ${name}=${value}; ${normalizedFallback} 사용`);
  return normalizedFallback;
}

function resolveStrictChoice(value, allowed, fallback, name) {
  const normalizedFallback = String(fallback).trim().toLowerCase();
  if (!allowed.includes(normalizedFallback)) {
    throw new Error(`[config] Invalid default ${name}=${fallback}`);
  }
  ...
  if (!allowed.includes(normalized)) {
    throw new Error(`[config] Invalid ${name}=${value}; ${allowed.join("|")}만 허용합니다.`);
  }
  return normalized;
}

function resolveNonEmptyString(envValue, defaultValue, name) {
  const raw = envValue !== undefined && envValue !== "" ? envValue : defaultValue;
  const value = String(raw).trim();
  if (!value) throw new Error(`[config] Invalid ${name}; 비어 있지 않은 값이 필요합니다.`);
  return value;
}

function resolvePositiveInteger(envValue, defaultValue, name) {
  const raw = envValue !== undefined && envValue !== "" ? envValue : defaultValue;
  const value = Number(raw);
  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(`[config] Invalid ${name}=${raw}; 양의 정수만 허용합니다.`);
  }
  return value;
}

function resolveOptionalPositiveNumber(envValue, defaultValue, name) {
  const raw = envValue !== undefined && envValue !== "" ? envValue : defaultValue;
  if (raw === undefined || raw === null || raw === "") return null;
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error(`[config] Invalid ${name}=${raw}; 양수만 허용합니다.`);
  }
  return value;
}
...
function partitionConfiguredRepos(configuredRepos, existsSync = fs.existsSync) {
  const repos = {};
  const unavailableRepos = {};
  for (const [name, def] of Object.entries(configuredRepos || {})) {
    const gitPath = path.join(def.path, ".git");
    if (existsSync(gitPath)) {
      repos[name] = def;
    } else {
      unavailableRepos[name] = {
        ...def,
        gitPath,
...
function loadConfig() {
  ...
  // 5. repos — 수집은 사용 가능한 checkout만 대상으로 하되, 누락 항목은
  // open-status 검증이 조용히 생략하지 않도록 별도 보존한다.
  const { repos, unavailableRepos } = partitionConfiguredRepos(raw.repos);
  ...
  const aiProvider = resolveStrictChoice(
    process.env.AI_PROVIDER,
    ["claude", "codex"],
    defaults.aiProvider || "claude",
    "AI_PROVIDER"
  );
  const defaultAiModel = aiProvider === "codex"
    ? defaults.codexModel || "gpt-5.6-sol"
    : defaults.aiModel || "sonnet";
  const env = {
    apiKey: process.env.REDMINE_API_KEY || "",
    githubToken: process.env.GITHUB_TOKEN || "",
    aiSummarize: process.env.AI_SUMMARIZE === "1",
    mode: process.env.MODE || "generate",
    weeklyPublishDepth: process.env.WEEKLY_PUBLISH_DEPTH ?? "2",
    slackBriefingChannelId: process.env.SLACK_BRIEFING_CHANNEL_ID || "",
    slackBriefingTeamId: process.env.SLACK_BRIEFING_TEAM_ID || "",
    slackBriefingBotId: process.env.SLACK_BRIEFING_BOT_ID || "",
    slackBriefingBotUserId: process.env.SLACK_BRIEFING_BOT_USER_ID || "",
    slackBriefingPeerUserId: process.env.SLACK_BRIEFING_PEER_USER_ID || "",
    ...
    aiProvider,
    aiGenerationMethod: resolveStrictChoice(
      process.env.AI_GENERATION_METHOD, ["freeform", "source_selection"],
      defaults.aiGenerationMethod || "freeform", "AI_GENERATION_METHOD"
    ),
    sourceSelectionFallback: resolveStrictChoice(
      process.env.SOURCE_SELECTION_FALLBACK, ["0", "1"], "1", "SOURCE_SELECTION_FALLBACK"
    ) === "1",
    claudeCli: process.env.CLAUDE_CLI || defaults.claudeCli || "claude",
    codexCli: process.env.CODEX_CLI || defaults.codexCli || "codex",
    aiModel: resolveNonEmptyString(
      process.env.AI_MODEL,
      defaultAiModel,
      "AI_MODEL"
    ),
    aiEffort: resolveChoice(
      process.env.AI_EFFORT,
      ["low", "medium", "high", "xhigh", "max"],
      defaults.aiEffort || "low",
      "AI_EFFORT"
    ),
    aiGenerationScope: resolveStrictChoice(
      process.env.AI_GENERATION_SCOPE,
      ["whole", "project"],
      defaults.aiGenerationScope || "whole",
      "AI_GENERATION_SCOPE"
    ),
    aiMaxInputChars: resolvePositiveInteger(
      process.env.AI_MAX_INPUT_CHARS,
      defaults.aiMaxInputChars ?? 100000,
      "AI_MAX_INPUT_CHARS"
    ),
    aiTimeoutMs: resolvePositiveInteger(
      process.env.AI_TIMEOUT_MS,
      defaults.aiTimeoutMs ?? 300000,
      "AI_TIMEOUT_MS"
    ),
    aiMaxBudgetUsd: resolveOptionalPositiveNumber(
      process.env.AI_MAX_BUDGET_USD,
      defaults.aiMaxBudgetUsd,
      "AI_MAX_BUDGET_USD"
    ),
