const fs = require("fs");
const { collectNotionItems, fetchNotionPage } = require("./collect-notion-api");
const { resolveReportReferences } = require("./report-references");
...
const { fetchPageMarkdown } = require("./notion-issue-publisher");
const {
  archiveSnapshot,
  buildSnapshotPath,
  readSnapshot,
...
async function hydrateNotionBriefings(items, candidates, config, apiKey, deps, warnings) {
  if (Number(config.env.reportDepth) < 3 || !apiKey) return items;
  const candidateIds = new Set((candidates || []).map(candidate => candidate.pageId).filter(Boolean));
  const hydrated = [];
  for (const item of items) {
    if (item.sourceDb !== "knowledgeBase" || !candidateIds.has(item.pageId)) {
      hydrated.push(item);
      continue;
    }
    try {
      const markdown = await deps.fetchPageMarkdown({ notionKey: apiKey }, item.pageId);
      const briefing = deps.buildNotionBriefing(markdown, { summary: item.summary });
      hydrated.push(Object.keys(briefing).length ? { ...item, briefing } : item);
    } catch (error) {
      warnings.push(`Notion 발표 상세 수집 실패: ${item.pageId} (${error.message})`);
      hydrated.push(item);
    }
  }
  return hydrated;
...
async function collectSnapshot(config, meetingDate, options = {}) {
  ...
  if (sourceEnabled(config, "notion", false)) {
    if (!process.env.NOTION_API_KEY && !options.notionApiKey) {
      ...
      try {
        ...
        if (notionItems.some((item) => item.timePrecision === "day")) {
          warnings.push("Notion date 속성 항목은 날짜 정밀도라 마지막 KST 날짜 전체를 포함함");
        }
      } catch (error) {
        notionStatus = "failed";
    ...
    console.log("[collect] notion: disabled");
  }
  ...
  if (sourceEnabled(config, "session", false)) {
    ...
    console.log("[collect] session: disabled");
  }
  ...
  notionItems = await hydrateNotionBriefings(
    notionItems,
    presentationCandidates,
    config,
    options.notionApiKey || process.env.NOTION_API_KEY,
    deps,
    warnings
  );
  // Explicit historical references are metadata, not new weekly activities.
  // A configured reference failure must not silently become a publishable partial run.
  notionItems = await resolveReportReferences(notionItems, config, pageId =>
    deps.fetchNotionPage(options.notionApiKey || process.env.NOTION_API_KEY, pageId));
  ...
  const emittedBlocks = {};
  const rawContent = deps.buildContent(meetingDate, autoContent, config, emittedBlocks);
  const sourceDetails = bindSummaryProvenance(
    autoContent, rawContent, summarySpans, config.repos, emittedBlocks
  );
