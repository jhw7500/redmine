...
function mergeItemsIntoAutoContent(gitResult, notionInput, sessionInput, config, dateRange) {
  ...
  function addItem(item, sourceTag) {
    ...
    const briefingTitle = normalizeForDedup(normalizeProjectName(item.title) || lines[0] || "");
...
module.exports = { filterByDateRange, mergeIntoAutoContent, mergeItemsIntoAutoContent };
