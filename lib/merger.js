...
function mergeItemsIntoAutoContent(gitResult, notionInput, sessionInput, config, dateRange, summarySpans = []) {
  ...
  function addItem(item, sourceTag) {
    ...
    for (const [itemLineIndex, line] of lines.entries()) {
      ...
      const references = sourceTag === "[Notion]" && !briefingAdded
        ? referenceLines(item, category === "etc" ? "    " : "      ") : [];
      // A linked duplicate can be selected alone, so retain its own conditions.
      const briefing = sourceTag === "[Notion]" && !briefingAdded
        ? notionBriefingLines(item, category)
          .filter((detail) => references.length > 0 || !briefingKeywords.has(
      ...
      if (summary.length && (references.length > 0 || !summaryKeywords.has(summaryKey))) {
        briefing.push(...summary);
        summaryKeywords.add(summaryKey);
        summaryAdded = true;
      }
      const referenceOffset = briefing.length;
      briefing.push(...references);
      ...
      if (references.length) {
        const parentLine = append ? existing.split("\n").length + 1 : 1;
        const parentLines = indent.split("\n");
        summarySpans.push({ kind: "report_reference", templateKey,
          sourceIndex: notionInput.indexOf(item), sourceHash: hashObject(item), itemLineIndex,
          parentLine, parentText: parentLines[0],
          startLine: parentLine + parentLines.length + referenceOffset,
          endLine: parentLine + parentLines.length + referenceOffset + references.length - 1,
        });
      }
