...
const FACT_LIKE_PREFIX = /\[[\s\u200B\uFEFF]*\[[\s\u200B\uFEFF]*fact[\s\u200B\uFEFF]*[:：]/iy;
const FACT_ID_PATTERN = /^[TQVS]\d{4}$/;

function hasFactLikePrefixAt(source, cursor) {
  FACT_LIKE_PREFIX.lastIndex = cursor;
  return FACT_LIKE_PREFIX.test(source);
}
...
function parseAnnotatedDraft(content) {
  ...
  while (cursor < source.length) {
    ...
    const factLikeMarker = !exactMarker
      && source[cursor] === "["
      && hasFactLikePrefixAt(source, cursor);
