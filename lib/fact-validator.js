...
function comparableQuantitySubject(subject) {
  ...
  const metricSubject = String(subject).replace(/\s+(?:개수|건수|수)$/, "").trim();
  return metricSubject ? nounKey(metricSubject) : null;
}
...
function claimQuantitySubject(claim, cleanContent) {
  ...
  return comparableQuantitySubject(
    occurrence.subject === undefined
...
function validateClaim(claim, byId, cleanContent, issues) {
  ...
  if (!fact.id.startsWith("Q") || !fact.subject) return;
  const expectedSubject = comparableQuantitySubject(fact.subject);
  const actualSubject = claimQuantitySubject(claim, cleanContent);
  if (actualSubject !== expectedSubject) {
    addIssue(
      issues,
      "error",
      "fact_subject_mismatch",
      `fact ID ${claim.factId}의 센 대상이 원본 근거와 다릅니다: ${actualSubject || "없음"}`,
      {
        value: claim.surface,
        factId: claim.factId,
...
function findUnmarkedProtectedFacts(cleanContent, markedCleanSpans, catalog, issues) {
  for (const occurrence of extractSchemaV2UnmarkedOccurrences(cleanContent, catalog)) {
    if (fullyContained(markedCleanSpans, occurrence.start, occurrence.end)) continue;
    addIssue(
      issues,
      "error",
