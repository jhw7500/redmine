...
function extractTestFactOccurrences(text) {
  ...
  for (const match of source.matchAll(standalone)) {
    ...
    if (!occupied(spans, start, end)) {
      facts.push({ pass: Number(match[1]), raw: match[0], ...locate(source, start, end) });
      spans.push({ start, end });
    }
  }
  return facts;
...
function closedSurfaceMatches(fact, surface) {
  ...
  if (fact.allowedNormalizations.includes("whitespace_around_slash")) {
    return surface.replace(/\s*\/\s*/g, "/") === fact.raw.replace(/\s*\/\s*/g, "/");
  }
  ...
  if (/^\d+(?:\.\d+)?\s*[x×]\s*\d+(?:\.\d+)?$/i.test(fact.raw)) {
    return surface.replace(/\s+/g, "").replace(/x/gi, "×")
      === fact.raw.replace(/\s+/g, "").replace(/x/gi, "×");
  }
...
function validateClaim(claim, byId, cleanContent, issues) {
  ...
  if (fact.type !== "counted_quantity") return;
  const counted = extractCountedQuantityOccurrences(cleanContent).find((occurrence) =>
    occurrence.start <= claim.cleanStart && occurrence.end >= claim.cleanEnd
  );
  if (counted && counted.subject !== fact.subject) {
    addIssue(
      issues,
      "error",
      "fact_subject_mismatch",
      `fact ID ${claim.factId}의 센 대상이 원본 근거와 다릅니다: ${counted.subject}`,
      {
        value: claim.surface,
        factId: claim.factId,
        expected: [fact.subject],
        actual: counted.subject,
        sourceLocation: fact.sourceLocation,
        sourceExcerpt: fact.sourceExcerpt,
        ...outputEvidence,
      }
...
function extractSchemaV2UnmarkedOccurrences(text, catalog) {
  const source = String(text);
  const occurrences = [
    ...extractTestFactOccurrences(source),
    ...extractProtectedTokenOccurrences(source),
    ...extractCountedQuantityOccurrences(source),
  ];

  for (const match of source.matchAll(/\b\d{4}-\d{2}-\d{2}\b/g)) {
    const start = match.index;
    occurrences.push({ raw: match[0], ...locate(source, start, start + match[0].length) });
  }

  for (const fact of catalog.facts.filter((candidate) => candidate.id.startsWith("S"))) {
    if (!fact.raw) continue;
    let start = source.indexOf(fact.raw);
    while (start !== -1) {
      occurrences.push({ raw: fact.raw, ...locate(source, start, start + fact.raw.length) });
      start = source.indexOf(fact.raw, start + fact.raw.length);
    }
  }
  ...
  for (const occurrence of occurrences.sort((left, right) =>
    ...
    if (!occupied(selected, occurrence.start, occurrence.end)) selected.push(occurrence);
  }
  return selected;
}

function findUnmarkedProtectedFacts(cleanContent, markedCleanSpans, catalog, issues) {
  for (const occurrence of extractSchemaV2UnmarkedOccurrences(cleanContent, catalog)) {
    if (occupied(markedCleanSpans, occurrence.start, occurrence.end)) continue;
    addIssue(
      issues,
      "error",
