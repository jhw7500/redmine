...
function extractProtectedTokens(text) {
  ...
  const patterns = [
    ...
    /\b\d+\s*[x×]\s*\d+\b/gi,
    /\b\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?\b/g,
    /\b\d+(?:\.\d+)?\s*(?:ms|us|ns|s|분|시간|Hz|kHz|MHz|GHz|Mbps|Gbps|MB|GB|건|개|회)\b/gi,
  ];
  for (const pattern of patterns) {
...
function validateReport(rawContent, reportContent, options = {}) {
  ...
  const underlineOpen = (String(reportContent).match(/<u>/g) || []).length;
  const underlineClose = (String(reportContent).match(/<\/u>/g) || []).length;
