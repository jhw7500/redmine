...
function validateClaim(claim, byId, cleanContent, issues) {
  ...
  const counted = extractSchemaV2FactOccurrences(cleanContent).find((occurrence) =>
    occurrence.type === "counted_quantity"
    && occurrence.start <= claim.cleanStart
    && occurrence.end >= claim.cleanEnd
  );
