...
async function generateSourceSelection(records, config, meetingDate, options) {
  ...
  let selection;
  if (!errorCode) {
    try { selection = parseSourceSelection(content, records, leaderHighlight); }
    catch (error) {
  ...
  return {
    content:renderSourceSelection(records, selection, {fallback:Boolean(errorCode),leaderHighlight}),
