...
function collectCandidates(source, systemFacts) {
  ...
  const addProtected = (pattern, prefix, type, kind) => addMatches(
    ...
    (match) => ({
      ...
      kind,
      semantic: {
        normalized: match[0].replace(/[ \t]+/g, "").toLowerCase(),
      },
    })
  );
  ...
  addProtected(
    /(?<![A-Za-z0-9_])[+-]?\d+(?:\.\d+)?[eE][+-]?\d+(?:[A-Za-z]+|[ \t]+(?:ms|us|ns|s|Hz|kHz|MHz|GHz|fps|bps|Kbps|Mbps|Gbps|bit|K|KB|MB|GB|B)|[ \t]*(?:초|분|시간|건|개|회|가지|번|차례))(?![A-Za-z0-9_가-힣])/gi,
    "Q",
    "measured_quantity",
    "scientific_notation"
  );
  ...
  addProtected(
    /(?<![A-Za-z0-9_])[+-]?\d+(?:\.\d+)?[ \t]*(?:ms|us|ns|s|초|분|시간|Hz|kHz|MHz|GHz|Mbps|Gbps|KB|MB|GB|B|건|개|회|가지|번|차례)(?![A-Za-z0-9_가-힣])/gi,
    "Q",
    "measured_quantity",
    "supported_unit"
  );
  addProtected(
    /(?<![A-Za-z0-9_])[+-]?\d+(?:\.\d+)?(?:°[A-Za-z]+|[℃℉]|-[A-Za-z]+|[A-Za-z]+)(?:\d+(?:\.\d+)?(?:°[A-Za-z]+|[℃℉]|-[A-Za-z]+|[A-Za-z]+))*(?![A-Za-z0-9_])/g,
    "Q",
    "measured_quantity",
    "attached_unit"
  );
...
function collectMarkerSuffixCandidates(source, markedSpans) {
  ...
  addProtected(
    /[+-]?\d+(?:\.\d+)?[ \t]*(?:ms|us|ns|s|초|분|시간|Hz|kHz|MHz|GHz|Mbps|Gbps|KB|MB|GB|B|건|개|회|가지|번|차례)(?![A-Za-z0-9_가-힣])/gi,
    "supported_unit"
  );
