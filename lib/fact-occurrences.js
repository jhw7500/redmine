...
function collectCandidates(source, systemFacts) {
  ...
  const countedPattern = /(\d+(?:\.\d+)?)\s*(개|건|회|가지|번|차례)(?=[\s의])\s*(?:의\s*)?([가-힣]+)/g;
  ...
  const bareCountedPattern = /\b(\d+(?:\.\d+)?)\s+([가-힣]+)(?=\s|$|[.,:;!?()[\]{}'"`])/g;
  ...
  addProtected(/\b\d+(?:\.\d+)?\s*%/g, "Q", "measured_quantity", "percentage");
  addProtected(/\b\d+(?:\.\d+)?\s*[x×]\s*\d+(?:\.\d+)?\b/gi, "Q", "measured_quantity", "dimension");
  addProtected(
    /\b\d+(?:\.\d+)?\s*(?:ms|us|ns|s|초|분|시간|Hz|kHz|MHz|GHz|Mbps|Gbps|KB|MB|GB|B|건|개|회|가지|번|차례)(?![A-Za-z0-9_가-힣])/gi,
    "Q",
    "measured_quantity",
    "supported_unit"
  );
  ...
  addProtected(/\bv?\d+(?:\.\d+){1,3}\b/gi, "V", "protected_token", "version");
  addProtected(/\b\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?\b/g, "V", "protected_token", "ratio");
  addProtected(/\b\d+(?:\.\d+)?\b/g, "Q", "quantity", "bare_number");
