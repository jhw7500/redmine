...
const VERSION_CORE_SOURCE = String.raw`\d+(?:\.\d+){1,3}`;
const VERSION_SUFFIX_SOURCE = String.raw`(?:[A-Za-z][A-Za-z0-9]*)?(?:-[A-Za-z0-9]+(?:[.-][A-Za-z0-9]+)*)?(?:\+[A-Za-z0-9]+(?:[.-][A-Za-z0-9]+)*)?`;
const NAMED_VERSION_TOKEN_SOURCE = String.raw`v?${VERSION_CORE_SOURCE}${VERSION_SUFFIX_SOURCE}`;
const FLEXIBLE_VERSION_TOKEN_SOURCE = String.raw`(?:v|[+-])?${VERSION_CORE_SOURCE}${VERSION_SUFFIX_SOURCE}`;
const NAMED_VERSION_PATTERN = new RegExp(
  String.raw`\b(GLIBC|wpa_supplicant)[ \t]+(${NAMED_VERSION_TOKEN_SOURCE}(?:[ \t]*\/[ \t]*${NAMED_VERSION_TOKEN_SOURCE})?)(?![\w.+-])`,
  "g"
);
const VERSION_RANGE_PATTERN = new RegExp(
  String.raw`(?<![A-Za-z0-9_])${FLEXIBLE_VERSION_TOKEN_SOURCE}[ \t]*\/[ \t]*${FLEXIBLE_VERSION_TOKEN_SOURCE}(?![\w.+-])`,
  "gi"
);
...
const CONNECTED_POSITIVE_UNIT_PATTERN = new RegExp(
  `(?<=${UNICODE_WORD_CHARACTER_SOURCE})\\+\\d+(?:\\.\\d+)?${HORIZONTAL_SPACE_SOURCE}*(?:${SUPPORTED_UNIT_SOURCE})(?!${UNICODE_WORD_CHARACTER_SOURCE})`,
  "gu"
);
...
const VERSIONED_IDENTIFIER_PATTERN = new RegExp(
  `(?<!${ASCII_IDENTIFIER_CHARACTER_SOURCE})[A-Za-z][A-Za-z0-9_-]*-\\d+(?:\\.\\d+){1,3}\\b(?!\\.\\d)`,
  "gu"
);
...
function collectCandidates(source, systemFacts) {
  const candidates = [];
  // Explicit software version labels, not arbitrary words followed by a decimal
  // (which could be a measured quantity). Bind the product and full release
  // token, including letter, prerelease and build suffixes, to one fact.
  for (const match of source.matchAll(NAMED_VERSION_PATTERN)) {
    const start = match.index + match[0].length - match[2].length;
    candidates.push({
      prefix: "V", type: "named_version", kind: "named_version",
      raw: match[2], subject: match[1], semantic: { version: match[2].replace(/^v/, "") },
      ...locate(source, start, start + match[2].length),
    });
  }
  ...
  addProtected(
    CONNECTED_POSITIVE_UNIT_PATTERN,
    "Q",
    "measured_quantity",
    "supported_unit"
  );
  ...
  addProtected(VERSION_RANGE_PATTERN, "V", "protected_token", "version");
  addProtected(/(?<![A-Za-z0-9_])(?:v|[+-])?\d+(?:\.\d+){1,3}\b/gi, "V", "protected_token", "version");
  addProtected(VERSIONED_IDENTIFIER_PATTERN, "V", "protected_token", "alphanumeric_identifier");
  addProtected(ALPHANUMERIC_IDENTIFIER_PATTERN, "V", "protected_token", "alphanumeric_identifier");
  // A ratio cannot start inside a dotted address/version (192.168.0.0/24).
  addProtected(/(?<![A-Za-z0-9_])(?<!\d\.)[+-]?\d+(?:\.\d+)?[ \t]*\/[ \t]*[+-]?\d+(?:\.\d+)?\b/g, "V", "protected_token", "ratio");
...
function collectMarkerSuffixCandidates(source, markedSpans, sourceCandidates) {
  // A complete device ID followed by a spaced unit name (MAX9296 fps) is not
  // a measurement of its numeric suffix. Numeric/version/hex markers still
  // require the existing suffix checks, and attached identifier edits still fail.
  const markedIdentifiers = sourceCandidates.filter(candidate =>
    candidate.kind === 'alphanumeric_identifier'
    && /^[A-Za-z][A-Za-z0-9_-]*\d$/.test(candidate.raw)
    && !/^v\d/i.test(candidate.raw)
    && fullyContained(markedSpans,candidate.start,candidate.end)
  );
  const candidates = [];
  ...
  return candidates.filter((candidate) =>
    ...
    && !markedIdentifiers.some(identifier =>
      identifier.start < candidate.start && candidate.start < identifier.end
      && /^[\t\p{Zs}]+/u.test(source.slice(identifier.end,candidate.end))
    )
  );
}
...
function extractSchemaV2FactOccurrences(text, options = {}) {
  ...
  const sourceCandidates = collectCandidates(source, systemFacts);
  const candidates = [
    ...sourceCandidates,
    ...collectMarkerSuffixCandidates(source, markedSpans, sourceCandidates),
  ]
    // Existing sealed catalogs may own GLIBC/wpa_supplicant versions as Q
    // quantities. Claim validation must be able to use that original grammar.
    .filter((candidate) => !(options.legacyCountedVersions && candidate.kind === "named_version"))
