...
function collectCandidates(source, systemFacts) {
  ...
  for (const match of source.matchAll(/\b(GLIBC|wpa_supplicant)[ \t]+(v?\d+(?:\.\d+){1,3}(?:[A-Za-z][A-Za-z0-9]*)?(?:-[A-Za-z0-9]+(?:[.-][A-Za-z0-9]+)*)?(?:\+[A-Za-z0-9]+(?:[.-][A-Za-z0-9]+)*)?)(?![\w.+-])/g)) {
  ...
  addProtected(
    /(?<![A-Za-z0-9_])(?:v|[+-])?\d+(?:\.\d+){1,3}[ \t]*\/[ \t]*(?:v|[+-])?\d+(?:\.\d+){1,3}(?![A-Za-z0-9_.])/gi,
    "V",
    "protected_token",
    "version"
  );
