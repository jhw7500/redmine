...
function topLevelLabel(line) {
  ...
  return cleanBulletLabel(match[1]);
}

function cleanBulletLabel(label) {
  return String(label)
...
function markdownBullet(line) {
  ...
  return {
    indentation: match[1],
    marker: match[2],
    label: cleanBulletLabel(match[3]),
  };
}
...
function outputRootBlock(output, expectedRoot, sectionHeader, knownRoots = [expectedRoot]) {
  ...
  for (let index = headerIndex + 1; index < lines.length; index += 1) {
    ...
    if (!bullet) {
      if (!/^\s/.test(line)) {
        throw scopeError(`${expectedRoot} 출력에 프로젝트 범위 밖 내용이 있습니다.`);
      }
      continue;
    }
    if (bullet.indentation.includes("\t")
      || bullet.indentation.length % 2 !== 0
      || bullet.marker !== "-") {
      throw scopeError(`${expectedRoot} 출력의 목록 들여쓰기 또는 불릿 마커가 올바르지 않습니다.`);
    }
    if (bullet.indentation.length === 0) {
      topLevelIndexes.push(index);
      rootSeen = true;
    } else if (!rootSeen) {
      throw scopeError(`${expectedRoot} 출력에 최상위 프로젝트 밖 내용이 있습니다.`);
    }
    if (knownRoots.includes(bullet.label) && bullet.label !== expectedRoot) {
      throw scopeError(`${expectedRoot} 출력에 다른 프로젝트가 있습니다: ${bullet.label}`);
    }
  }
...
function mergeProjectOutputs(outputs, sectionHeader, knownRoots) {
  if (!Array.isArray(outputs) || outputs.length === 0) {
    throw scopeError("병합할 프로젝트 출력이 없습니다.");
  }
  const roots = Array.isArray(knownRoots) ? knownRoots : outputs.map(({ id }) => id);
  const blocks = outputs.map(({ id, content }) => (
    outputRootBlock(content, id, sectionHeader, roots)
  ));
...
function validateProjectOutput({ id, content }, rawContent, config, options = {}) {
  try {
    mergeProjectOutputs([{ id, content }], config.env.sectionHeader, projectRoots(config));
  } catch (error) {
