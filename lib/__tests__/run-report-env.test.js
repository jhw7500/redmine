...
function isolatedEnv(fixture, overrides = {}) {
  return {
    ...
    SLACK_BOT_TOKEN: "",
    SLACK_CHANNEL_ID: "",
    SLACK_BRIEFING_CHANNEL_ID: "",
    SLACK_BRIEFING_TEAM_ID: "",
    SLACK_BRIEFING_BOT_ID: "",
    SLACK_BRIEFING_BOT_USER_ID: "",
    SLACK_BRIEFING_PEER_USER_ID: "",
    ...overrides,
  };
}

test('pair wrapper preserves explicit depth selection over dotenv and defaults to depth2', t => {
  const fixture = copyRuntimeFixture(t);
  const capturePath = path.join(fixture.dir, 'pair-capture.json');
  writeNodeExecutable(path.join(fixture.binDir, 'bash'), `
    const args = process.argv.slice(2);
    if (args.length === 1 && require('node:path').basename(args[0]) === 'run-report-env.sh') {
      const result = require('node:child_process').spawnSync('/usr/bin/bash', args, {env:process.env,stdio:'inherit'});
      process.exit(result.status === null ? 1 : result.status);
    }
    require('node:fs').writeFileSync(process.env.CAPTURE_PATH, JSON.stringify({
      mode:process.env.MODE,depth:process.env.WEEKLY_PUBLISH_DEPTH,profileDepth:process.env.REPORT_DEPTH,
      date:process.env.MEETING_DATE,output:process.env.OUTPUT_DIR,
      briefingRecipient:process.env.SLACK_BRIEFING_CHANNEL_ID || '',
      botFromRepowire:process.env.SLACK_BOT_TOKEN === 'xoxb-fixture-repowire',
    }));
  `);
  writeFixtureEnv(fixture.root, 'MODE=wrong\nWEEKLY_PUBLISH_DEPTH=3\nMEETING_DATE=2000-01-01\nOUTPUT_DIR=wrong\n');
  fs.mkdirSync(path.join(fixture.homeDir, '.repowire'));
  fs.writeFileSync(path.join(fixture.homeDir, '.repowire/config.yaml'),
    'slack:\n  bot_token: xoxb-fixture-repowire\n  channel_id: CSHARED\n');
  for (const args of [['publish'], ['publish','3'], ['prepare'], ['preview'], ['send']]) {
    const result = spawnSync('/usr/bin/bash', [path.join(fixture.root,'run-weekly-pair-env.sh'), ...args], {
      encoding:'utf8', env:isolatedEnv(fixture,{CAPTURE_PATH:capturePath,MEETING_DATE:'2026-09-09',OUTPUT_DIR:fixture.dir}),
    });
    assert.equal(result.status, 0, result.stderr);
    const capture = JSON.parse(fs.readFileSync(capturePath));
    assert.equal(capture.mode, 'weekly-pair-' + args[0]);
    assert.equal(capture.depth, args[1] || '2');
    assert.equal(capture.profileDepth, '2');
    assert.equal(capture.date, '2026-09-09');
    assert.equal(capture.output, fixture.dir);
    assert.equal(capture.briefingRecipient, '');
    if (args[0] === 'send') assert.equal(capture.botFromRepowire, true);
  }
  const bad = spawnSync('/usr/bin/bash', [path.join(fixture.root,'run-weekly-pair-env.sh'),'publish','4'], {
    encoding:'utf8',env:isolatedEnv(fixture),
  });
...
`);
  ...
  const { buildWeeklyPipelinePaths, createWeeklyAttempt, recordWeeklyFailure } = require("../weekly-pipeline");
  const state = createWeeklyAttempt({ outputDir, meetingDate: "2026-09-16", reportDepth: 3,
    attemptId: "11111111-1111-4111-8111-111111111111", now: () => "2026-09-16T06:05:00.000Z" });
  recordWeeklyFailure({ paths: buildWeeklyPipelinePaths(outputDir, "2026-09-16"), state,
    stage: "collect", error: new Error("fixture prepare failed"), artifacts: [],
    redmineWriteAttempted: false, serverState: "unchanged",
    retryCommand: "rtk env MEETING_DATE=2026-09-16 ./run-weekly-prepare-env.sh",
    now: () => "2026-09-16T06:06:00.000Z" });
  ...
  const statusPath = path.join(meetingDir, "status.json");
  const invalidBytes = JSON.stringify({ status: "failed" });
  fs.writeFileSync(statusPath, invalidBytes);
  const invalidPublish = run("run-weekly-publish-env.sh");
  assert.strictEqual(invalidPublish.status, 1, invalidPublish.stderr);
  assert.match(invalidPublish.stderr, /\[weekly\]\[FAIL\].*code=terminal_state_invalid/);
  assert.strictEqual(alertLines().length, 3);
  assert.strictEqual(fs.readFileSync(statusPath, "utf8"), invalidBytes);

  const lockCalls = fs.readFileSync(lockCapturePath, "utf8").trim().split("\n").map(JSON.parse);
  assert.strictEqual(lockCalls.length, 4);
