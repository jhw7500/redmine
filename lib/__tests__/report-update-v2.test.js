...
const { writeJsonAtomic } = require("../report-artifact");
const { setupCompleteV2Run } = require("./helpers/report-run-fixture");
...
test("schema v2 update rechecks mutable open-issue evidence before Redmine", async (t) => {
  ...
  const fixture = await setupCompleteV2Run(t, {
    rawContent: `${HEADER}\n${openLine}\n`,
    aiOutput: `${HEADER}\n- \`arg.cam[i].bps\` 설정 오류 (2026-05-[[fact:Q0001|08]] 기준 미해결)\n`,
...
test("schema v2 assertReady rechecks canonical content immediately before PUT", async (t) => {
  let changed = false;
  const fixture = await setupCompleteV2Run(t, {
    onRedmineRequest: (request, current) => {
      if (!changed && request.method === "GET") {
        changed = true;
        fs.writeFileSync(current.reportPath, `${HEADER}\n- raced canonical edit\n`, "utf8");
      }
    },
  });
...
test("schema v2 assertReady preserves snapshot failure code before PUT", async (t) => {
  let changed = false;
  const fixture = await setupCompleteV2Run(t, {
    onRedmineRequest: (request, current) => {
      if (!changed && request.method === "GET") {
        changed = true;
        const state = JSON.parse(fs.readFileSync(current.generationStatePath, "utf8"));
        writeJsonAtomic(current.generationStatePath, {
          ...state,
