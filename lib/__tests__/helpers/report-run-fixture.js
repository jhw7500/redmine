...
function makeProjectFakeClaude(fixture, outputsByRoot, options = {}) {
  const executablePath = path.join(fixture.dir, "fake-project-claude.js");
  return writeExecutable(executablePath, [
    'const fs = require("node:fs");',
    `const callsPath = ${JSON.stringify(fixture.callsPath)};`,
    'const calls = Number(fs.readFileSync(callsPath, "utf8"));',
    'fs.writeFileSync(callsPath, String(calls + 1), "utf8");',
    ...(options.supersedeGenerationStatePath ? [
      'if (calls === 0) {',
      `  fs.writeFileSync(${JSON.stringify(options.supersedeGenerationStatePath)}, JSON.stringify({`,
      '    schemaVersion: 2,',
      '    attemptId: "newer-test-attempt",',
      '    status: "running"',
      '  }), "utf8");',
      '}',
    ] : []),
    ...
    `const outputs = ${JSON.stringify(outputsByRoot)};`,
    'const sourceLines = source.split("\\n");',
    'const root = Object.keys(outputs).find((candidate) => sourceLines.some((line) => (',
    '  line === "- " + candidate || line.startsWith("- " + candidate + " [[source:")',
    ')));',
    'if (!root) { process.stderr.write("project root not found"); process.exit(2); }',
    'process.stdout.write(outputs[root]);',
    "",
  ].join("\n"));
}
...
async function attachWikiServer(t, fixture, options = {}) {
  ...
  let pageVersion = 1;
  let updatedOn = "2026-08-26T09:00:00Z";
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      if (request.method === "PUT") {
        ...
        pageText = payload.wiki_page.text;
        pageVersion += 1;
        updatedOn = "2026-08-26T09:05:00Z";
      }
...
async function setupCompleteV2Run(t, options = {}) {
  ...
  fixture.config.env.presentationNoteMode = options.presentationNoteMode || "off";
  fixture.config.env.aiSummarize = true;
  fixture.config.env.aiProvider = options.aiProvider || "claude";
  if (options.aiGenerationMethod) {
    fixture.config.env.aiGenerationMethod = options.aiGenerationMethod;
  }
  if (options.sourceSelectionFallback !== undefined) {
    fixture.config.env.sourceSelectionFallback = options.sourceSelectionFallback;
  }
  fixture.config.env.aiModel = options.aiModel
    || (fixture.config.env.aiProvider === "codex" ? "gpt-5.6-sol" : "sonnet");
  const fakeAiCli = makeFakeClaude(
    fixture,
    options.aiOutput || rawContent
  );
  if (fixture.config.env.aiProvider === "codex") fixture.config.env.codexCli = fakeAiCli;
  else fixture.config.env.claudeCli = fakeAiCli;
