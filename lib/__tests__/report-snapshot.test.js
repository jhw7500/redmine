...
test("depth 3 snapshots preserve source-grounded cause, fix, and verification details", async () => {
  ...
  try {
    ...
    deps.fetchPageMarkdown = async () => [
      ...
      blocksToMd([{
        type: "paragraph",
        paragraph: { rich_text: [{ plain_text: "실기 검증:", annotations: {} }] },
      }, {
        type: "code",
        code: {
          language: "markdown",
    ...
    deps.buildContent = (_date, autoContent, _config, emittedBlocks) => {
      const block = autoContent["{{CAMERA}}"];
      emittedBlocks["{{CAMERA}}"] = { offset: 3, length: block.split("\n").length };
      return [
        "#### 조현우",
        "- PIM",
        "  - Camera Driver",
