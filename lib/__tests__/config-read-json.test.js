...
test("source selection is explicit, fallback is configurable, and typos fail closed", () => {
  withEnv({MODE:'collect',AI_GENERATION_METHOD:undefined,SOURCE_SELECTION_FALLBACK:undefined}, () => {
    assert.strictEqual(loadConfig().env.aiGenerationMethod,'freeform');
    assert.strictEqual(loadConfig().env.sourceSelectionFallback,true);
    process.env.AI_GENERATION_METHOD = 'source_selection';
    process.env.SOURCE_SELECTION_FALLBACK = '0';
    assert.strictEqual(loadConfig().env.aiGenerationMethod,'source_selection');
    assert.strictEqual(loadConfig().env.sourceSelectionFallback,false);
    process.env.AI_GENERATION_METHOD = 'typo';
    assert.throws(()=>loadConfig(),/Invalid AI_GENERATION_METHOD/);
    process.env.AI_GENERATION_METHOD = 'source_selection';
    process.env.SOURCE_SELECTION_FALLBACK = 'typo';
    assert.throws(()=>loadConfig(),/Invalid SOURCE_SELECTION_FALLBACK/);
  });
});
...
test("repository filtering retains unavailable entries for open-status verification", () => {
  const configured = {
    available: { path: "/repos/available" },
    missing: { path: "/repos/missing" },
  };
  const result = partitionConfiguredRepos(
    configured,
    (gitPath) => gitPath === "/repos/available/.git"
  );

  assert.deepStrictEqual(result.repos, { available: configured.available });
  assert.deepStrictEqual(result.unavailableRepos, {
    missing: {
      ...configured.missing,
      gitPath: "/repos/missing/.git",
...
test("revalidate exposes RUN_ID without requiring a Redmine key", () => {
  ...
  withEnv({ MODE: "revalidate", RUN_ID: attemptId, REDMINE_API_KEY: undefined }, () => {
    const config = loadConfig();
    assert.strictEqual(config.env.mode, "revalidate");
    assert.strictEqual(config.env.runId, attemptId);
  });
});

test("run artifact retention has a 90-day dry-run default", () => {
  withEnv({
    MODE: "prune",
    RUN_ARTIFACT_RETENTION_DAYS: undefined,
    PRUNE_APPLY: undefined,
    REDMINE_API_KEY: undefined,
  }, () => {
    const config = loadConfig();
    assert.strictEqual(config.env.runArtifactRetentionDays, 90);
    assert.strictEqual(config.env.pruneApply, false);
  });
});

test("run artifact retention accepts an explicit age and apply flag", () => {
  withEnv({
    MODE: "prune",
    ...
    const config = loadConfig();
    assert.strictEqual(config.env.runArtifactRetentionDays, 30);
    assert.strictEqual(config.env.pruneApply, true);
  });
});

test("run artifact retention rejects a zero-day policy", () => {
  withEnv({
    MODE: "prune",
    RUN_ARTIFACT_RETENTION_DAYS: "0",
    PRUNE_APPLY: undefined,
    REDMINE_API_KEY: undefined,
  }, () => {
    assert.throws(() => loadConfig(), /RUN_ARTIFACT_RETENTION_DAYS.*양의 정수/);
  });
});
...
test("AI execution budget has low-cost fail-closed defaults", () => {
  withAiEnv({}, () => {
    ...
    assert.strictEqual(config.env.codexCli, "codex");
  });
});
...
test("AI execution budget accepts explicit environment overrides", () => {
  withAiEnv(
    ...
    () => {
      ...
      assert.strictEqual(config.env.aiGenerationScope, "project");
      ...
      assert.strictEqual(config.env.claudeCli, "/opt/claude");
    }
  );
});

test("Codex selection uses its own model and CLI defaults", () => {
  withAiEnv({ AI_PROVIDER: "codex" }, () => {
    const config = loadConfig();

    assert.strictEqual(config.env.aiProvider, "codex");
    assert.strictEqual(config.env.aiModel, "gpt-5.6-sol");
    assert.strictEqual(config.env.codexCli, "codex");
  });
});

test("invalid AI provider and generation scope fail before spawn", () => {
  withAiEnv({ AI_PROVIDER: "gemini" }, () => {
    assert.throws(() => loadConfig(), /AI_PROVIDER/);
  });
  withAiEnv({ AI_GENERATION_SCOPE: "auto" }, () => {
    assert.throws(() => loadConfig(), /AI_GENERATION_SCOPE/);
  });
});
