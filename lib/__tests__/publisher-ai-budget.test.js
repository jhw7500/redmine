...
test("Codex summary uses a read-only ephemeral exec and reads the prompt from stdin", async (t) => {
  const cliPath = makeFakeClaude(
    t,
    `let input = "";
...
process.stdin.on("end", () => {
  ...
  const config = createConfig("unused-claude", {
    aiProvider: "codex",
    codexCli: cliPath,
    aiModel: "gpt-5.6-sol",
    aiEffort: "medium",
  ...
  const result = JSON.parse(
    await aiSummarize("원본 보고서", config, new Date("2026-08-26T00:00:00"))
  );

  assert.deepStrictEqual(result.args.slice(0, 4), [
    "exec",
    "--ephemeral",
    "--ignore-user-config",
    "--ignore-rules",
  ...
  assert.deepStrictEqual(
    result.args.slice(result.args.indexOf("--sandbox"), result.args.indexOf("--sandbox") + 2),
    ["--sandbox", "read-only"]
  );
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    result.args.slice(result.args.indexOf("-c"), result.args.indexOf("-c") + 2),
    ["-c", 'model_reasoning_effort="medium"']
  );
  assert.ok(result.args.includes("--skip-git-repo-check"));
  assert.strictEqual(result.args.at(-1), "-");
  assert.ok(!result.args.includes("-p"));
  assert.match(result.input, /원본 보고서/);
  assert.match(path.basename(result.cwd), /^redmine-codex-/);
  assert.strictEqual(fs.existsSync(result.cwd), false);
});
...
test("inline fact prompt forbids synthesizing combined phase labels", () => {
  ...
  assert.match(prompt, /reference 앞뒤의 대상 구절.*함께 그대로 복사/);
});
...
test("project generation calls each configured root once and merges one section", async (t) => {
  const callsPath = path.join(os.tmpdir(), `redmine-ai-project-calls-${process.pid}-${Date.now()}`);
  t.after(() => fs.rmSync(callsPath, { force: true }));
  const cliPath = makeFakeClaude(
    t,
    `const fs = require("node:fs");
const args = process.argv.slice(2);
const prompt = args[args.indexOf("-p") + 1];
const source = prompt.slice(prompt.lastIndexOf("원본:\\n") + "원본:\\n".length);
const roots = ["PIM", "Wireless Lan", "ETC"];
const root = roots.find((candidate) => source.includes("- " + candidate + "\\n"));
fs.appendFileSync(${JSON.stringify(callsPath)}, root + "\\n", "utf8");
process.stdout.write(${JSON.stringify('#### <span style="color:blue">조현우</span>\n')} + "- " + root + "\\n  - " + root + " 결과\\n");`
  );
  const projectConfig = createConfig(cliPath, {
  ...
  assert.strictEqual(fs.readFileSync(callsPath, "utf8"), "PIM\nWireless Lan\nETC\n");
  ...
  assert.strictEqual(result.generationPlan.scope, "project");
  assert.deepStrictEqual(
    result.generationPlan.calls.map((call) => call.id),
    ["PIM", "Wireless Lan", "ETC"]
  );
});
