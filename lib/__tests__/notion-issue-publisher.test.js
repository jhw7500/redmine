...
test("presentation-note HTTP helpers omit private response bodies from structured errors", async (t) => {
  for (const scenario of ["GET HTTP", "POST HTTP", "PUT HTTP", "GET malformed JSON", "POST malformed issue"]) {
    await t.test(scenario, async (t) => {
      const requests = [];
      const server = http.createServer(async (request, response) => {
        for await (const _chunk of request) { /* consume the real request body */ }
        requests.push(request.method);
        response.setHeader("Content-Type", "application/json");
        if (scenario === "PUT HTTP" && request.method === "GET") {
          response.end(JSON.stringify(request.url.includes("parent_id=")
            ? { issues: [], total_count: 0 }
            : { issue: { id: 42, status: { id: 1 }, allowed_statuses: [{ id: 9 }] } }));
        } else if (scenario === "POST malformed issue") {
          response.writeHead(201);
          response.end(JSON.stringify({ privateBody: "PRIVATE_NOTE_BODY_SENTINEL" }));
        } else {
          response.writeHead(scenario === "GET malformed JSON" ? 200 : 500);
      ...
      const env = { redmineBase: baseUrl, redmineKey: "fixture-key" };
      const request = scenario.startsWith("GET") ? () => fetchJson(`${baseUrl}/metadata`)
        : scenario.startsWith("POST") ? () => createIssue(env, CREATE_META, CREATE_NOTE, "fixture", "2026-08-26")
          : () => closeIssue(env, 42, 9);
      await assert.rejects(request, (error) => {
        assert.doesNotMatch(`${error.message}\n${error.stack}\n${JSON.stringify(error)}`, /PRIVATE_/);
        assert.strictEqual(error.code, scenario === "GET malformed JSON" ? "PRESENTATION_JSON_INVALID"
          : scenario === "POST malformed issue" ? "PRESENTATION_RESPONSE_INVALID" : "PRESENTATION_HTTP_FAILED");
        if (scenario !== "POST malformed issue") {
          assert.strictEqual(error.status, scenario === "GET malformed JSON" ? 200 : 500);
          assert.match(error.message, /HTTP (200|500)/);
        }
        return true;
      });
      assert.deepStrictEqual(requests, scenario === "PUT HTTP" ? ["GET", "GET", "PUT"]
        : [scenario.startsWith("POST") ? "POST" : "GET"]);
    });
  }
});
...
test("publishNotes checks readiness immediately before every issue creation", async (t) => {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (target.includes("/projects/p.json")) {
      ...
      issuePosts += 1;
      evidenceValid = false;
      body = { issue: { id: 100 + issuePosts, status: { id: 9 }, assigned_to: { id: 10 } } };
  ...
  await assert.rejects(
    ...
    (error) => error.message === "publish evidence invalid" && error.serverState === "written_unverified"
  );
...
test("createIssue rejects when meta carries no statusId to verify against", async (t) => {
  stubCreateIssueFetch(t, { id: 4823, status: { id: 9, name: "검토" }, assigned_to: { id: 10 } });
  await assert.rejects(
...
test("publishNotes flags a reused issue whose status differs from the requested one", async (t) => {
  const calls = stubPublishFetch(t, {
    post: { id: 9999, status: { id: 9 }, assigned_to: { id: 10 } },
...
test("queryCompletedNotes treats a never-used tag as zero completed notes", async (t) => {
  ...
  global.fetch = async () => ({
    ...
    text: async () => JSON.stringify({
      ...
      message: 'multi_select option "발표완료" not found for property "tags". Available options: "발표노트".',
    }),
    json: async () => ({}),
  });
  const notes = await queryCompletedNotes(CLOSE_PIPE_ENV);
...
// #64 — assigned_to_id는 사전 검사도 사후 검증도 없던 유일한 필드였다.
// 대상이 프로젝트의 할당 가능 멤버가 아니면 조용히 미할당으로 생성될 수 있다.

test("createIssue rejects when the created issue carries a different assignee", async (t) => {
  stubCreateIssueFetch(t, { id: 4823, status: { id: 9 }, assigned_to: { id: 99 } });
  await assert.rejects(
    () => createIssue(CREATE_ENV, CREATE_META, CREATE_NOTE, "# 본문", "2026-09-02"),
    /assigned_to_id 미반영/
  );
...
test("createIssue rejects when the assignee was dropped from the response", async (t) => {
  stubCreateIssueFetch(t, { id: 4823, status: { id: 9 } });
  await assert.rejects(
    () => createIssue(CREATE_ENV, CREATE_META, CREATE_NOTE, "# 본문", "2026-09-02"),
    /assigned_to_id 미반영/
  );
...
test("createIssue reports every mismatched field, not just the first", async (t) => {
  stubCreateIssueFetch(t, { id: 4823, status: { id: 1 }, assigned_to: { id: 99 } });
  await assert.rejects(
    () => createIssue(CREATE_ENV, CREATE_META, CREATE_NOTE, "# 본문", "2026-09-02"),
    (err) => {
      assert.deepStrictEqual(err.fields, ["status_id", "assigned_to_id"]);
      assert.match(err.message, /status_id 미반영/);
      assert.match(err.message, /assigned_to_id 미반영/);
      return true;
    }
  );
});

test("publishNotes flags both fields when both were dropped", async (t) => {
  stubPublishFetch(t, { post: { id: 4823, status: { id: 1 }, assigned_to: null } });
  const refs = await publishNotes(PUBLISH_ENV, PUBLISH_NOTES);
  assert.strictEqual(refs[0].statusMismatch, true);
  assert.strictEqual(refs[0].assigneeMismatch, true,
    "확인하지 못한 필드를 '이상 없음'으로 보고하면 안 된다");
});

test("publishNotes keeps an assignee-mismatched issue in refs and flags it", async (t) => {
  stubPublishFetch(t, { post: { id: 4823, status: { id: 9 }, assigned_to: null } });
  const refs = await publishNotes(PUBLISH_ENV, PUBLISH_NOTES);
  assert.strictEqual(refs.length, 1, "생성된 이슈가 보고서 참조에서 누락되면 안 된다");
  assert.strictEqual(refs[0].id, 4823);
  assert.strictEqual(refs[0].assigneeMismatch, true);
...
test("publishNotes marks a fully applied issue as matching on both fields", async (t) => {
  stubPublishFetch(t, { post: { id: 4823, status: { id: 9 }, assigned_to: { id: 10 } } });
  const refs = await publishNotes(PUBLISH_ENV, PUBLISH_NOTES);
  assert.strictEqual(refs[0].statusMismatch, false);
  assert.strictEqual(refs[0].assigneeMismatch, false);
});
