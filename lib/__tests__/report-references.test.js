...
test('on-request references retain their access policy without checking artifact viewer access', async () => {
  const input = [source()];
  const entry = binding({ audience: 'on_request', version: 'v1' });
  const before = structuredClone({ input, entry });
  const requested = [];
  const result = await resolveReportReferences(input, cfg([entry]), async id => {
    requested.push(id); return page();
  });
  assert.deepEqual(requested, [pageId], 'only the saved reference metadata is fetched');
  assert.deepEqual(result[0].reportReferences, [{ referencePageId: pageId, label: '상세분석',
    url: artifactUrl, audience: 'on_request', version: 'v1', pageLastEditedTime: '2026-09-01T00:00:00Z' }]);
  assert.deepEqual({ input, entry }, before);
});

test('on-request links do not bypass report exclusions, unsafe URLs or unavailable metadata', async () => {
  for (const items of [[], [source({ reportExcluded: true })], [source({ report: 'private' })]]) {
    assert.deepEqual(await resolveReportReferences(items, cfg([binding({ audience: 'on_request' })]),
      async () => assert.fail('excluded source must not fetch')), items);
  }
  await assert.rejects(resolveReportReferences([source()], cfg([binding({ audience: 'on_request',
    url: 'https://example.com/doc?token=secret' })]), async () => assert.fail('unsafe URL must not fetch')),
  { code: 'REPORT_REFERENCE_INVALID' });
  await assert.rejects(resolveReportReferences([source()], cfg([binding({ audience: 'on_request' })]),
    async () => page({ properties: { ...page().properties, report: { type: 'select', select: { name: 'private' } } } })),
  { code: 'REPORT_REFERENCE_INVALID' });
  await assert.rejects(resolveReportReferences([source()], cfg([binding({ audience: 'on_request' })]),
    async () => { throw new Error('secret upstream body'); }),
  error => error.code === 'REPORT_REFERENCE_UNAVAILABLE' && !error.message.includes('secret'));
});

test('mixed access policies label only on-request links and keep all links in pairs', () => {
  assert.deepEqual(referenceLines({ reportReferences: [
    { label: '공유된 자료', url: 'https://example.com/team', audience: 'team' },
    { label: '개인 분석', url: 'https://example.com/personal', audience: 'on_request', version: 'v2' },
    { label: '추가 분석', url: 'https://example.com/extra', audience: 'on_request' },
  ] }, '  '), [
    '  ↳ 자료: [공유된 자료](https://example.com/team) · [개인 분석 (v2) · 요청 시 공유](https://example.com/personal)',
    '  ↳ 자료: [추가 분석 · 요청 시 공유](https://example.com/extra)',
  ]);
});
...
for (const audience of ['team', 'on_request']) for (const depth of [2, 3]) test(`depth${depth} generated report retains exact artifact URL, caveat and publish evidence (${audience})`, async t => {
  const f = await collected(t, depth, [source()], [binding({ audience })]);
  ...
  const label = audience === 'team' ? '상세분석' : '상세분석 · 요청 시 공유';
  assert.ok(reportContent.includes(`↳ 자료: [${label}](${artifactUrl})`), reportContent);
  ...
  assert.equal(records.records[0].continuations.find(line => line.kind === 'report_reference').references[0].audience, audience);
});
...
test('on-request audience and sharing-label tampering invalidate source evidence', async t => {
  const f = await collected(t, 3, [source()], [binding({ audience: 'on_request' })]);
  const coverage = buildSourceCoverageCatalog(f.snapshot, categories);
  for (const mutate of [
    snapshot => { snapshot.sources.notion.data[0].reportReferences[0].audience = 'team'; },
    snapshot => { snapshot.rawContent = snapshot.rawContent.replace(' · 요청 시 공유', ''); },
  ]) {
    const altered = structuredClone(f.snapshot); mutate(altered);
    assert.throws(() => buildSourceRecords(altered, altered.rawContent, coverage), { code: 'SOURCE_RECORDS_INVALID' });
  }
...
for (const audience of ['team', 'on_request']) test(`sealed snapshot reuse does not silently change references or fetch pages again (${audience})`, async t => {
  const f = await collected(t, 3, [source()], [binding({ audience })]);
  f.config.sources.notion.reportReferences[0].url = 'https://example.com/new-version';
  f.config.sources.notion.reportReferences[0].audience = audience === 'team' ? 'on_request' : 'team';
  ...
  assert.equal(result.snapshot.sources.notion.data[0].reportReferences[0].audience, audience);
});
...
for (const audience of ['team', 'on_request']) test(`paired outputs and saved mobile links are independently verified; altered link metadata blocks preview (${audience})`, async t => {
  const f = await collected(t, 3, [source()], [binding({ audience })]);
  ...
  const label = audience === 'team' ? '상세분석' : '상세분석 · 요청 시 공유';
  for (const report of originals) assert.ok(report.toString().includes(`[${label}](${artifactUrl})`));
  ...
  assert.equal(mobile.replies[0].blocks.at(-1).elements[0].elements[1].text, label);
  ...
  const altered = records.records[0].continuations.find(line => line.kind === 'report_reference').references[0];
  if (audience === 'on_request') altered.audience = 'team';
  else altered.url = 'https://example.com/wrong';
