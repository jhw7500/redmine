...
test("standalone carriage-return summary lines keep their source prefix", async t => {
  const f = await collected(t, [item({ summary: "done\r- injected" })]);
  const records = buildSourceRecords(f.snapshot, f.snapshot.rawContent,
    buildSourceCoverageCatalog(f.snapshot, categories));
  const rendered = renderSourceSelection(records, buildFallbackSelection(records));

  assert.doesNotMatch(f.snapshot.rawContent, /\r/);
  assert.doesNotMatch(rendered, /\r/);
  assert.match(rendered, /출처 요약: done\n\s+↳ 출처 요약: - injected/);
  assert.equal(f.snapshot.sourceDetails.spans[0].endLine
    - f.snapshot.sourceDetails.spans[0].startLine + 1, 2);
});
...
test("identical rendered summaries bind to their own template occurrences", async t => {
  const repeatedCategories = {
    app: { parent: "PIM", label: "Application", templateKey: "APP" },
    firmware: { parent: "PIM", label: "Firmware", templateKey: "FIRMWARE" },
  };
  const f = await collected(t, [
    item({ pageId: "app-page", category: "app" }),
    item({ pageId: "firmware-page", category: "firmware" }),
  ], repeatedCategories);

  assert.equal(f.snapshot.autoContent["{{APP}}"], f.snapshot.autoContent["{{FIRMWARE}}"]);
  assert.deepEqual(f.snapshot.sourceDetails.spans.map(span => span.templateKey), [
    "{{APP}}", "{{FIRMWARE}}",
  ]);
  assert.deepEqual(f.snapshot.sourceDetails.spans.map(span => span.parentLine), [4, 7]);
  assert.deepEqual(f.snapshot.sourceDetails.blocks, {
    "{{APP}}": { offset: 3, length: 2 },
    "{{FIRMWARE}}": { offset: 6, length: 2 },
  });
  const records = buildSourceRecords(f.snapshot, f.snapshot.rawContent,
    buildSourceCoverageCatalog(f.snapshot, repeatedCategories));
  assert.equal(records.records.length, 2);
  assert.deepEqual(records.records.map(record => record.continuations
    .filter(continuation => continuation.kind === "notion_summary").length), [1, 1]);
});

test("collection rejects static summary text when its template placeholder was not emitted", async t => {
  const f = setupSnapshot(t, { rawContent: "unused", categories });
  f.meetingDate = new Date("2026-09-09T00:00:00+09:00");
  f.snapshotPath = path.join(f.dir, "new-snapshot.json");
  Object.assign(f.config.env, {
    snapshotPath: f.snapshotPath, templatePath: path.join(f.dir, "template.md"),
    presentationNoteThreshold: 1000,
  });
  f.config.sources = { notion: { enabled: true }, git: { enabled: false }, session: { enabled: false } };
  fs.writeFileSync(f.config.env.templatePath, [
    f.config.env.sectionHeader,
    "- PIM",
    "  - Application",
    "    - [Notion] 서비스 복구",
    "      ↳ 출처 요약: 서비스를 복구했고 재발 방지 검사를 추가했다.",
  ...
  await assert.rejects(collectSnapshot(f.config, f.meetingDate, {
    notionApiKey: "fixture-notion-key",
    deps: { collectNotionItems: async () => [item()] },
...
test("summary provenance rejects shifted, partial, overlapping, or incorrectly bound source spans", async t => {
  ...
  const mutations = [
    ...
    s => { s.sourceDetails.blocks["{{APP}}"].offset++; },
    s => { s.sourceDetails.blocks["{{APP}}"].length++; },
    s => { delete s.sourceDetails.blocks["{{APP}}"]; },
    s => { s.sourceDetails.blocks = []; },
