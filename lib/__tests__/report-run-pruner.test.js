...
test("an expired AI failure is removed after its recovery window", (t) => {
  const outputDir = makeOutputDir(t);
  const runDir = writeRun(outputDir, {
    attemptId: OLD_ATTEMPT_ID,
    startedAt: "2026-05-01T00:00:00.000Z",
...
test("an AI failure inside the retention window is retained", (t) => {
  const outputDir = makeOutputDir(t);
  const runDir = writeRun(outputDir, {
    attemptId: NEW_ATTEMPT_ID,
    date: "2026-08-01",
  ...
  assert.deepStrictEqual(summary.skippedReasons, { within_retention: 1 });
...
test("a date directory replacement cannot redirect recursive deletion", (t) => {
  ...
  try {
    ...
    // Remove the link itself. Node 24 rmSync rejects this directory symlink
    // without recursive:true, which is unnecessary and unsafe for this cleanup.
    if (fs.lstatSync(dateDir).isSymbolicLink()) fs.unlinkSync(dateDir);
...
test("dry-run reports a held validation lock as locked", async (t) => {
  const outputDir = makeOutputDir(t);
  const runDir = writeRun(outputDir, {
    attemptId: OLD_ATTEMPT_ID,
    startedAt: "2026-05-01T00:00:00.000Z",
...
test("dry-run rejects a FIFO validation lock without blocking", (t) => {
  const outputDir = makeOutputDir(t);
  const runDir = writeRun(outputDir, {
    attemptId: OLD_ATTEMPT_ID,
    startedAt: "2026-05-01T00:00:00.000Z",
  ...
  assert.strictEqual(spawnSync("mkfifo", [lockPath]).status, 0);
