const { test } = require("node:test");
const assert = require("node:assert");
const { richToMd, blocksToMd } = require("../notion-blocks-to-md");

test("richToMd: plain/bold/code/link", () => {
  const rich = [
    { plain_text: "a ", annotations: {}, href: null },
    { plain_text: "bold", annotations: { bold: true }, href: null },
    { plain_text: " ", annotations: {}, href: null },
    { plain_text: "code", annotations: { code: true }, href: null },
    { plain_text: " ", annotations: {}, href: null },
    { plain_text: "link", annotations: {}, href: "http://x" },
  ];
  assert.strictEqual(richToMd(rich), "a **bold** `code` [link](http://x)");
});

test("blocksToMd: heading/para/list/code", () => {
  const blocks = [
    { type: "heading_2", heading_2: { rich_text: [{ plain_text: "제목", annotations: {} }] } },
    { type: "paragraph", paragraph: { rich_text: [{ plain_text: "본문", annotations: {} }] } },
    { type: "bulleted_list_item", bulleted_list_item: { rich_text: [{ plain_text: "항목", annotations: {} }] } },
    { type: "code", code: { language: "bash", rich_text: [{ plain_text: "iw reg get", annotations: {} }] } },
    { type: "divider", divider: {} },
  ];
  const md = blocksToMd(blocks);
  assert.match(md, /## 제목/);
  assert.match(md, /\n본문\n/);
  assert.match(md, /- 항목/);
  assert.match(md, /```bash\niw reg get\n```/);
  assert.match(md, /\n---\n/);
});

for (const { name, chunks, expected } of [
  {
    name: "nested triple-backtick example",
    chunks: ["```js\nconst example = 1;\n```\n## 검증\n- 예시 결과"],
    expected: "````markdown\n```js\nconst example = 1;\n```\n## 검증\n- 예시 결과\n````\n",
  },
  {
    name: "longer inner closing delimiter",
    chunks: ["```js\nexample\n```````"],
    expected: "````````markdown\n```js\nexample\n```````\n````````\n",
  },
  {
    name: "indented inner delimiter",
    chunks: ["  ```\n## 검증\n- 예시 결과"],
    expected: "````markdown\n  ```\n## 검증\n- 예시 결과\n````\n",
  },
  {
    name: "backtick run split across rich-text chunks",
    chunks: ["``", "``\n## 검증"],
    expected: "`````markdown\n````\n## 검증\n`````\n",
  },
]) {
  test(`blocksToMd preserves code boundaries for ${name}`, () => {
    const markdown = blocksToMd([{
      type: "code",
      code: {
        language: "markdown",
        rich_text: chunks.map(plain_text => ({ plain_text, annotations: {} })),
      },
    }]);

    assert.strictEqual(markdown, expected);
  });
}

test("blocksToMd: table with header row", () => {
  const row = (a, b) => ({ type: "table_row", table_row: { cells: [[{ plain_text: a, annotations: {} }], [{ plain_text: b, annotations: {} }]] } });
  const blocks = [{ type: "table", table: {}, __children: [row("k", "v"), row("1", "2")] }];
  const md = blocksToMd(blocks);
  assert.match(md, /\| k \| v \|/);
  assert.match(md, /\| --- \| --- \|/);
  assert.match(md, /\| 1 \| 2 \|/);
});

test("blocksToMd: unsupported fallback", () => {
  const md = blocksToMd([{ type: "callout", callout: { rich_text: [{ plain_text: "hi", annotations: {} }] } }]);
  assert.match(md, /<!-- unsupported: callout -->/);
});

test("blocksToMd: nested list children preserved (indented)", () => {
  const blocks = [{
    type: "bulleted_list_item",
    bulleted_list_item: { rich_text: [{ plain_text: "부모", annotations: {} }] },
    __children: [
      { type: "bulleted_list_item", bulleted_list_item: { rich_text: [{ plain_text: "자식", annotations: {} }] } },
    ],
  }];
  const md = blocksToMd(blocks);
  assert.match(md, /- 부모/);
  assert.match(md, /\n {2}- 자식/);
});

test("blocksToMd: null block skipped, table cell newline flattened", () => {
  const row = (a, b) => ({ type: "table_row", table_row: { cells: [[{ plain_text: a, annotations: {} }], [{ plain_text: b, annotations: {} }]] } });
  const md = blocksToMd([
    null,
    { type: "table", table: {}, __children: [row("h1", "h2"), row("a\nb", "c")] },
  ]);
  assert.match(md, /\| h1 \| h2 \|/);
  assert.match(md, /\| a b \| c \|/); // 개행이 공백으로
  assert.doesNotMatch(md, /a\nb/);
});
