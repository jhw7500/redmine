...
test("depth3 production prompt keeps its own budget without common shape rules", () => {
  ...
  assert.match(prompt, /3줄 이상이면 flat 나열 금지/);
  assert.match(prompt, /Application.*설정 안정화.*잘못된 배열 거부/s);
...
test("production prompt preserves pending work instead of upgrading it to completed", () => {
  const prompt = promptForDepth(3);

  assert.match(prompt, /예정·계획·필요·해야 함·진행 중·보류·미완료.*완료형으로 바꾸지 않는다/);
  assert.match(prompt, /전달·배포·적용·등록.*동작 명사.*실행 완료를 추론하지 않는다/);
  assert.match(prompt, /완료 항목과 미완료 항목.*대상별 상태를 분리해 보존/);
  assert.match(prompt, /상태가 불명확하면 완료로 단정하지 않고 원문 표현을 유지/);
});
