...
test("revalidate leaves the canonical report untouched when validation still fails", async (t) => {
  ...
  assert.strictEqual(state.validationRevision, 2);
  assert.strictEqual(state.latestRejectedReportPath, "report.rejected.002.md");
  assert.match(state.latestRejectedReportHash, /^[0-9a-f]{64}$/);
  assert.strictEqual(
    fs.existsSync(path.join(fixture.runPaths.runDir, "report.rejected.001.md")),
    true
  );
  assert.strictEqual(
    fs.existsSync(path.join(fixture.runPaths.runDir, "report.rejected.002.md")),
    true
  );
});
