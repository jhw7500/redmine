...
test("parser rejects fact-like markers with split brackets and Unicode spacing", () => {
  const markers = [
    "[ [fact:T0001]]",
    "[\t[fact:T0001]]",
    "[[\nfact:T0001]]",
  ...
  for (const marker of markers) {
    const parsed = parseAnnotatedDraft(marker);
    assert.deepStrictEqual(parsed.errors.map((error) => error.code), [
      "malformed_fact_marker",
    ], JSON.stringify(marker));
    assert.strictEqual(parsed.claims.length, 0, JSON.stringify(marker));
    assert.strictEqual(parsed.cleanContent, marker, JSON.stringify(marker));
  }
});

test("parser rejects Markdown-escaped fact-like markers", () => {
  const markers = [
    "[\\[fact:T0001]]",
    "\\[\\[fact:T0001]]",
    "[[fact\\:T0001]]",
