...
function fixture(t) {
  ...
  return {briefing,deliveryDir:path.join(dir,'delivery'),channel:'DPRIVATE1',token:'xoxb-PRIVATE_TEST_TOKEN',
    approvedIdentity:{teamId:'TAPPROVED',botId:'BAPPROVED',botUserId:'UAPPBOT',peerUserId:'UOWNER'},
    assertSource:() => {},sleep:async () => {}};
}

function verifiedFetch(f, post) {
  return async (url, options) => {
    if (url.endsWith('/auth.test')) return new Response(JSON.stringify({
      ok:true,team_id:f.approvedIdentity.teamId,bot_id:f.approvedIdentity.botId,
      user_id:f.approvedIdentity.botUserId,
    }));
    if (url.endsWith('/conversations.info')) return new Response(JSON.stringify({
      ok:true,channel:{id:f.channel,is_im:true,is_mpim:false,is_ext_shared:false,
        user:f.approvedIdentity.peerUserId},
    }));
...
test('Slack sends a parent and one-column thread, then skips identical delivery without keeping secrets', async t => {
  const f = fixture(t);
  const requests = [];
  const fetch = verifiedFetch(f, async (url, options) => {
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(f.deliveryDir,'manifest.json'),'utf8'));
  assert.deepEqual({teamId:manifest.teamId,botId:manifest.botId,
    botUserId:manifest.botUserId,peerUserId:manifest.peerUserId},f.approvedIdentity);
...
for (const failure of ['timeout','http','api','wrong-channel','bad-ts','truncated']) {
  test(`Slack ${failure} keeps uncertain delivery evidence and blocks blind resend`, async t => {
    const f = fixture(t);
    let calls = 0;
    const fetch = verifiedFetch(f, async () => {
      calls++;
...
test('Slack persists newly created journal ancestors before the first HTTP request', async t => {
  ...
  let calls = 0;
  await sendSlackBriefing({...f,fetch:verifiedFetch(f,async () => {
...
test('Slack directory sync failure makes zero HTTP requests', async t => {
  ...
  let calls = 0;
  await assert.rejects(sendSlackBriefing({...f,fetch:verifiedFetch(f,async () => {
    calls++;
...
test('Slack rejects an unapproved workspace identity before posting report content', async t => {
  const f = {...fixture(t),approvedIdentity:{
    teamId:'TAPPROVED',botId:'BAPPROVED',botUserId:'UAPPBOT',peerUserId:'UOWNER',
  }};
  const urls = [];
  await assert.rejects(sendSlackBriefing({...f,fetch:async url => {
    urls.push(url);
    if (url.endsWith('/auth.test')) return new Response(JSON.stringify({
      ok:true,team_id:'TOTHER',bot_id:'BAPPROVED',user_id:'UAPPBOT',
    }));
    if (url.endsWith('/conversations.info')) return new Response(JSON.stringify({
      ok:true,channel:{id:f.channel,is_im:true,user:'UOWNER'},
    }));
    return new Response(JSON.stringify({ok:true,channel:f.channel,ts:'1234567890.000001'}));
  }}), /identity/i);
  assert.equal(urls.some(url=>url.endsWith('/chat.postMessage')),false);
});

test('Slack rejects widened journal directory permissions before network', async t => {
  const f = fixture(t);
  fs.mkdirSync(f.deliveryDir,{mode:0o700});
  fs.chmodSync(f.deliveryDir,0o750);
  let calls = 0;
