...
test("every independently delimited open-status clause requires its own as-of date", () => {
  const report = "TODO A (2026-08-28 기준 미해결), TODO B";
  const result = validate("A와 B 조사", report);

  assert.strictEqual(result.status, "FAIL");
  assert.deepStrictEqual(
    result.issues
      .filter((issue) => issue.code === "open_status_without_as_of")
  ...
  assert.strictEqual(
    validate(
      "A와 B 조사",
      "TODO A (2026-08-28 기준 미해결), TODO B (2026-08-28 기준 미해결)"
    ).status,
    "PASS"
  );
  assert.strictEqual(
    validate("대량 1,000건 점검", "TODO 1,000건 점검 (2026-08-28 기준 미완료)").status,
    "PASS"
  );
  assert.strictEqual(
    validate("코드 점검", "TODO `foo,bar` 점검 (2026-08-28 기준 미완료)").status,
    "PASS"
  );
  assert.deepStrictEqual(
    validate("A와 B 조사", "TODO A (2026-08-28 기준 미해결) / TODO B").issues
      .filter((issue) => issue.code === "open_status_without_as_of")
      .map((issue) => issue.line),
    ["TODO B"]
  );
  ...
  const evasions = [
    "TODO A (2026-08-28 기준 미해결)\\, TODO B",
    "TODO A (2026-08-28 기준 미해결) \\/ TODO B",
    "TODO A (2026-08-28 기준 미해결) `note / TODO B",
    "TODO A (2026-08-28 기준 미해결) `note / TODO B`",
    "TODO A (2026-08-28 기준 미해결) (note / TODO B",
  ];
  for (const evasion of evasions) {
    assert.ok(
      validate("A와 B 조사", evasion).issues.some(
        (issue) => issue.code === "open_status_without_as_of" && issue.line.includes("TODO B")
      ),
      evasion
    );
  }
