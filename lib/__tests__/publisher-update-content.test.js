  ...
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      if (putSucceeded && options.postPutStatus) {
        ...
        response.end(JSON.stringify({ error: "post-write verification failed" }));
        return;
      }
...
test("update returns structured remote evidence and invokes callbacks when no PUT is needed", async (t) => {
  ...
  assert.deepStrictEqual(events, ["BEFORE_EXTERNAL_WRITE", "FINAL_SECTION"]);
...
test("update marks successful writes unverified and does not persist when the final GET fails", async (t) => {
  ...
  for (const scenario of [
    { name: "HTTP failure", server: { postPutStatus: 500 } },
    ...
    await t.test(scenario.name, async (t) => {
      ...
      await assert.rejects(
        ...
        (error) => {
          assert.deepStrictEqual(Object.keys(error).sort(), [
