...
test('ETC project names matching commit-type labels remain in the prompt and report', () => {
  for (const project of ['기타','개선']) {
    const snapshot = {contentHash:'snapshot',autoContent:{'{{ETC}}':'- updated'},rawContent:[
      '#### 조현우','- ETC',`  - ${project}`,'    - 오류 반환 경로 개선','',
    ].join('\n')};
    const coverage = buildSourceCoverageCatalog(snapshot,{etc:{parent:'ETC',label:null,templateKey:'ETC'}});
    const records = buildSourceRecords(snapshot,snapshot.rawContent,coverage);
    assert.deepEqual(records.records[0].context.map(parent=>parent.text),[project]);
    assert.ok(buildSelectionPrompt(records,{env:{reportDepth:3}}).includes(`"text":"${project}"`));
    assert.ok(renderSourceSelection(records,buildFallbackSelection(records)).includes(`    - ${project}\n      - 오류 반환 경로 개선`));
  }
});

test('commit-type words deeper than the category grouping retain their source context', () => {
  const snapshot = {contentHash:'snapshot',autoContent:{'{{APP}}':'- updated'},rawContent:[
    '#### 조현우','- PIM','  - Application','    - 변경 범위','      - 개선','        - 오류 반환 경로 개선','',
  ].join('\n')};
  const coverage = buildSourceCoverageCatalog(snapshot,{app:{parent:'PIM',label:'Application',templateKey:'APP'}});
  const records = buildSourceRecords(snapshot,snapshot.rawContent,coverage);
  assert.deepEqual(records.records[0].context.map(parent=>parent.text),['변경 범위','개선']);
});
...
test('selection rejects rewrites, unknown IDs, cross-category moves, duplicate IDs, and omissions', () => {
  ...
  // 펜스는 벗겨지고 그 안의 내용이 검증된다 — 코드만 보면 이 구분이 드러나지 않아 메시지도 고정한다.
  assert.throws(() => parseSourceSelection('```json\n{}\n```', records),
    {code:'SOURCE_SELECTION_INVALID', message:'Expected sections only'});
});

test('selection parsing rejects an unterminated fence without superlinear backtracking', () => {
  const { records } = fixture();
  // \uc5f4\ub9ac\ub294 \ud39c\uc2a4 \ub4a4\uc5d0 \uac1c\ud589\ub9cc \uc774\uc5b4\uc9c0\uace0 \ub2eb\ud788\uc9c0 \uc54a\ub294 \uc785\ub825. \uc774\ucc28 \ubc31\ud2b8\ub798\ud0b9\uc774\uba74 \uc218 \ucd08 \uac78\ub9b0\ub2e4.
  const hostile = '```json\n' + '\n'.repeat(64000) + 'x';
  const started = process.hrtime.bigint();
  assert.throws(() => parseSourceSelection(hostile, records), {code:'SOURCE_SELECTION_INVALID'});
  const elapsedMs = Number(process.hrtime.bigint() - started) / 1e6;
  assert.ok(elapsedMs < 1000, `unterminated fence took ${elapsedMs.toFixed(1)}ms`);
});

test('selection parsing accepts a JSON body wrapped in a markdown code fence', () => {
  const { records } = fixture();
  const expected = parseSourceSelection(JSON.stringify(selection()), records);
  const fenced = '```json\n' + JSON.stringify(selection()) + '\n```';
  assert.deepEqual(parseSourceSelection(fenced, records), expected);
  assert.deepEqual(parseSourceSelection('```\n' + JSON.stringify(selection()) + '\n```', records), expected);
  // \ud39c\uc2a4\ub97c \ubc97\uaca8\ub3c4 \ub0b4\uc6a9 \uac80\uc99d\uc740 \uadf8\ub300\ub85c\ub2e4.
...
function depthThreeFixture() {
  const sections = Array.from({length:4},(_,index)=>({
    ...
    path:['PIM',`Section ${index+1}`],headings:['PIM',`Section ${index+1}`],
  }));
  const records = sections.flatMap((section,sectionIndex)=>Array.from({length:9},(_,itemIndex)=>{
    const number = sectionIndex*9+itemIndex+1;
    return {id:`R${String(number).padStart(4,'0')}`,sectionId:section.id,text:`항목 ${number}`,
      annotatedText:`항목 ${number}`,context:[],continuations:[]};
  }));
  return {heading:'#### 조현우',sections,records};
}

function selectionWithCounts(catalog, counts) {
  const themes = ['stability','verification','delivery'];
  return {sections:catalog.sections.map((section,index)=>{
    const items = catalog.records.filter(record=>record.sectionId===section.id).slice(0,counts[index]);
    return {id:section.id,groups:Array.from({length:Math.ceil(items.length/3)},(_,groupIndex)=>({
      theme:themes[groupIndex],items:items.slice(groupIndex*3,groupIndex*3+3)
        .map(record=>({id:record.id,highlight:false})),
    }))};
  })};
}

test('depth 3 validates 24 to 32 work items overall without equal per-section quotas', () => {
  const records = depthThreeFixture();
  assert.throws(()=>parseSourceSelection(
    JSON.stringify(selectionWithCounts(records,[1,3,3,4])),records,undefined,3
  ),{code:'SOURCE_SELECTION_INVALID'});
  for (const counts of [[1,5,9,9],[5,9,9,9]]) {
    const selected = selectionWithCounts(records,counts);
    assert.deepEqual(parseSourceSelection(JSON.stringify(selected),records,undefined,3),selected);
  }
  assert.throws(()=>parseSourceSelection(
    JSON.stringify(selectionWithCounts(records,[6,9,9,9])),records,undefined,3
  ),{code:'SOURCE_SELECTION_INVALID'});
});

test('depth 3 prompt and fallback target 24 work items rather than counting headings', () => {
  const records = depthThreeFixture();
  const fallback = buildFallbackSelection(records,3);
  const selectedCount = fallback.sections.flatMap(section=>section.groups)
    .flatMap(group=>group.items).length;
  assert.equal(selectedCount,24);
  assert.match(buildSelectionPrompt(records,{env:{reportDepth:3}}),/전체 실제 업무 항목 24~32개/);
});

test('depth 3 fallback preserves late and unevenly distributed detail records before filling titles', () => {
  const catalog = depthThreeFixture();
  const details = catalog.records.slice(1,9);
  for (const record of details) record.continuations = [{text:'↳ 원인: 입력 경계 불일치',annotatedText:'↳ 원인: 입력 경계 불일치'}];
  const late = {...catalog.records[0],id:'R0037',text:'늦게 수집된 상세',annotatedText:'늦게 수집된 상세',
    continuations:[{text:'↳ 수정: 경계 통일',annotatedText:'↳ 수정: 경계 통일'},
      {text:'↳ 검증: 재현 시험 통과',annotatedText:'↳ 검증: 재현 시험 통과'}]};
  catalog.records.push(late);
  const before = JSON.stringify(catalog);
  const selected = buildFallbackSelection(catalog,3);
  const ids = selected.sections.flatMap(section=>section.groups.flatMap(group=>group.items.map(item=>item.id)));
  for (const record of [...details,late]) assert.ok(ids.includes(record.id),`missing ${record.id}`);
  assert.equal(ids.length,24);
  assert.deepEqual(parseSourceSelection(JSON.stringify(selected),catalog,undefined,3),selected);
  assert.deepEqual(buildFallbackSelection(catalog,'3'),selected);
  assert.equal(JSON.stringify(catalog),before);
  const rendered = renderSourceSelection(catalog,selected,{fallback:true,reportDepth:3});
  for (const line of late.continuations) assert.ok(rendered.includes(line.text));
});

test('depth 3 fallback grows beyond its minimum to retain details and parent explanations', () => {
  const catalog = depthThreeFixture();
  const details = catalog.records.filter((record,index)=>index%9<7);
  for (const record of details) record.context = [{text:'장치 조건',annotatedText:'장치 조건',
    continuations:[{text:'외부 전원 연결 시에만 적용',annotatedText:'외부 전원 연결 시에만 적용'}]}];
  const selected = buildFallbackSelection(catalog,3);
  const ids = selected.sections.flatMap(section=>section.groups.flatMap(group=>group.items.map(item=>item.id)));
  assert.deepEqual(new Set(ids),new Set(details.map(record=>record.id)));
  assert.equal(ids.length,28);
  assert.doesNotThrow(()=>renderSourceSelection(catalog,selected,{fallback:true,reportDepth:3}));
});

test('detail prioritization does not change other fallback depths or ordinary AI selections', () => {
  const catalog = depthThreeFixture();
  const original = [undefined,1,2,4].map(depth=>buildFallbackSelection(catalog,depth));
  for (const record of catalog.records.slice(6)) record.continuations = [{text:'상세 설명',annotatedText:'상세 설명'}];
  assert.deepEqual([undefined,1,2,4].map(depth=>buildFallbackSelection(catalog,depth)),original);
  const ai = selectionWithCounts(catalog,[6,6,6,6]);
  assert.deepEqual(parseSourceSelection(JSON.stringify(ai),catalog,undefined,3),ai);
});

test('selection rejects disabled or excessive highlights across all sections', () => {
  const {records} = fixture();
  const value = selection();
  value.sections[1].groups[0].items[0].highlight = true;
  const json = JSON.stringify(value);
  assert.throws(()=>parseSourceSelection(json,records,{enabled:false,maxLines:0}),{code:'SOURCE_SELECTION_INVALID'});
  assert.throws(()=>parseSourceSelection(json,records,{enabled:true,maxLines:1}),{code:'SOURCE_SELECTION_INVALID'});
  assert.deepEqual(parseSourceSelection(json,records,{enabled:true,maxLines:2}),value);
  assert.deepEqual(parseSourceSelection(json,records,{enabled:true,maxLines:0}),value);
  assert.throws(()=>renderSourceSelection(records,value,{leaderHighlight:{enabled:false,maxLines:0}}),{code:'SOURCE_SELECTION_INVALID'});
});

test('highlight prompts use domain-neutral criteria when guidance is absent or blank', () => {
  const {records} = fixture();
  for (const guidance of [undefined, '', ' \n ', null, 42]) {
    const prompt = buildSelectionPrompt(records, {
      env:{reportDepth:3},reportFilter:{leaderHighlight:{enabled:true,maxLines:2,guidance}},
    }).split('원본 레코드:')[0];
    assert.doesNotMatch(prompt, /PIM|Wireless Lan/);
    assert.match(prompt, /주요 성과|마일스톤/);
    assert.match(prompt, /장애|일정 리스크/);
    assert.match(prompt, /최대 2개/);
  }
});

test('highlight prompts follow configured eligible domains without adding a fixed restriction', () => {
  const {records} = fixture();
  const guidance = 'ETC의 주간보고 자동화 성과만 강조한다.';
  const prompt = buildSelectionPrompt(records, {
    env:{reportDepth:3},reportFilter:{leaderHighlight:{enabled:true,maxLines:1,guidance:`  ${guidance}  `}},
  }).split('원본 레코드:')[0];
  assert.ok(prompt.includes(`\n${guidance}\n`));
  assert.doesNotMatch(prompt, /PIM|Wireless Lan/);
  assert.match(prompt, /최대 1개/);
});
