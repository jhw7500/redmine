...
test("prune mode is dry-run by default and deletes only with PRUNE_APPLY=1", (t) => {
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "redmine-prune-mode-"));
  t.after(() => fs.rmSync(outputDir, { recursive: true, force: true }));
  const attemptId = "11111111-1111-4111-8111-111111111111";
  const runDir = path.join(outputDir, "runs", "2026-05-01", attemptId);
  ...
  const baseEnv = {
    ...
    RUN_ARTIFACT_RETENTION_DAYS: "90",
  };

  const preview = spawnSync(process.execPath, [indexPath], {
    cwd: path.resolve(__dirname, "../.."),
    env: { ...baseEnv, PRUNE_APPLY: "0" },
    encoding: "utf8",
  });
  assert.strictEqual(preview.status, 0, preview.stderr);
  assert.match(
    preview.stdout,
    /\[prune\] mode=dry-run retentionDays=90 examined=1 eligible=1 deleted=0 skipped=0 errors=0/
  );
  assert.strictEqual(fs.existsSync(runDir), true);

  const apply = spawnSync(process.execPath, [indexPath], {
    cwd: path.resolve(__dirname, "../.."),
    env: { ...baseEnv, PRUNE_APPLY: "1" },
    encoding: "utf8",
  });
  assert.strictEqual(apply.status, 0, apply.stderr);
  assert.match(
    apply.stdout,
    /\[prune\] mode=apply retentionDays=90 examined=1 eligible=1 deleted=1 skipped=0 errors=0/
  );
  assert.strictEqual(fs.existsSync(runDir), false);
});

test("weekly prepare profile is exact while generic modes retain their existing defaults", () => {
  const env = {
    aiSummarize: true,
    aiProvider: "codex",
    aiModel: "gpt-5.6-sol",
  ...
  assert.doesNotThrow(() => assertWeeklyProfile({ env }, "weekly-prepare"));
  assert.doesNotThrow(() => assertWeeklyProfile({ env: { ...env, reportDepth: 2 } }, "weekly-prepare"));
  assert.doesNotThrow(() => assertWeeklyProfile({}, "generate"));
  ...
  for (const [field, value] of Object.entries({
    ...
    sourceSelectionFallback: false,
    reportDepth: 4,
    validationMode: "warn",
    validationOverride: true,
    presentationNoteMode: "tagged",
  })) {
    assert.throws(
      () => assertWeeklyProfile({ env: { ...env, [field]: value } }, "weekly-prepare"),
      new RegExp(field),
      field
    );
  }
...
test("weekly prepare accepts the claude profile and still pins its model", () => {
  const claudeEnv = {
    aiSummarize: true,
    aiProvider: "claude",
    aiModel: "sonnet",
  ...
  assert.doesNotThrow(() => assertWeeklyProfile({ env: claudeEnv }, "weekly-prepare"));
  assert.doesNotThrow(() => assertWeeklyProfile({ env: { ...claudeEnv, reportDepth: 2 } }, "weekly-prepare"));
  assert.throws(
    () => assertWeeklyProfile({ env: { ...claudeEnv, aiModel: "gpt-5.6-sol" } }, "weekly-prepare"),
    /aiModel=sonnet/
  );
  assert.throws(
    () => assertWeeklyProfile({ env: { ...claudeEnv, aiProvider: "gemini" } }, "weekly-prepare"),
    /aiProvider=codex or claude/
  );
  // \ud504\ub85c\ud1a0\ud0c0\uc785 \uc0c1\uc18d \ud0a4\ub294 truthy \ub77c \uadf8\ub0e5 \uc870\ud68c\ud558\uba74 provider \uac00\ub4dc\ub97c \uadf8\ub0e5 \uc9c0\ub098\uac04\ub2e4.
  for (const inherited of ["constructor", "__proto__", "toString", "valueOf", "hasOwnProperty"]) {
    assert.throws(
      () => assertWeeklyProfile({ env: { ...claudeEnv, aiProvider: inherited } }, "weekly-prepare"),
      /aiProvider=codex or claude/,
      inherited
    );
  }
...
test("main recognizes weekly-prepare and rejects a wrong profile before collecting", (t) => {
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "redmine-weekly-mode-"));
  t.after(() => fs.rmSync(outputDir, { recursive: true, force: true }));
  const indexPath = path.resolve(__dirname, "../../index.js");
  ...
  assert.match(result.stderr, /weekly-prepare requires aiSummarize=true/);
...
test("schema v2 cannot override a coverage catalog hash mismatch", () => {
  assert.throws(
    () => assertPublishable(
      {
        schemaVersion: 2,
        status: "FAIL",
        issues: [{ severity: "error", code: "coverage_catalog_hash_mismatch" }],
      },
      { env: { validationMode: "block", validationOverride: true } }
    ),
