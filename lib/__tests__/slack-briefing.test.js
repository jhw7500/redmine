...
test('on-request mobile links retain their visible sharing notice without changing team links or source prose',()=>{
  const references=[{label:'공유 자료',url:'https://example.com/team',audience:'team'},
    {label:'개인 분석',url:'https://example.com/personal',audience:'on_request',version:'v2'}];
  const records={sections:[{id:'C1',path:['WLAN']}],records:[{id:'R1',sectionId:'C1',text:'분석 결과',context:[],
    continuations:[{text:'조건은 그대로 보존한다.'},{kind:'report_reference',
      text:'↳ 자료: [공유 자료](https://example.com/team) · [개인 분석 (v2) · 요청 시 공유](https://example.com/personal)',
      references}]}]};
  const before=structuredClone(records);
  const input={meetingDate:'2026-09-09',reportHash:'a'.repeat(64),records,
    selection:{sections:[{id:'C1',groups:[{items:[{id:'R1'}]}]}]}};
  const preview=buildSlackBriefing(input);
  const reply=preview.replies[0];
  const links=reply.blocks.filter(b=>b.type==='rich_text').flatMap(b=>b.elements)
    .flatMap(s=>s.elements).filter(e=>e.type==='link');
  assert.deepEqual(links,[{type:'link',text:'공유 자료',url:'https://example.com/team'},
    {type:'link',text:'개인 분석 (v2) · 요청 시 공유',url:'https://example.com/personal'}]);
  assert.equal(reply.blocks[1].text.text,'분석 결과\n\n조건은 그대로 보존한다.');
  assert.match(reply.text,/개인 분석 \(v2\) · 요청 시 공유/);
  assert.equal(reply.unfurl_links,false);
  assert.equal(reply.mrkdwn,false);
  assert.deepEqual(records,before);
  references[1].audience='team';
  assert.throws(()=>buildSlackBriefing(input),/reference/i);
});
