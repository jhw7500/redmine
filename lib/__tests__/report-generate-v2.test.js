...
const { isPublishable, runGenerate } = require("../../index");
...
test("v2 promotes a publishable warning and keeps generate exit semantics successful", async (t) => {
  const fixture = setupSnapshot(t, { rawContent: `${HEADER}\n- 연결 안정화 작업\n` });
  fixture.config.env.aiSummarize = true;
  fixture.config.env.claudeCli = makeFakeClaude(
    fixture,
  ...
  assert.strictEqual(
    fs.readFileSync(fixture.reportPath, "utf8"),
    `${HEADER}\n- 연결 안정화 작업 (2026-08-26 기준 미완료)\n`
  );
  assert.strictEqual(
