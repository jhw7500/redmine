...
async function makeTerminalFixture(t, status) {
  let fixture;
  if (status === "failed") {
    ...
    fixture = { dir: outputDir, date: MEETING_DATE, meetingDate: new Date(MEETING_DATE),
      paths, config: publishConfig(outputDir) };
  } else {
    fixture = await setupWeeklyReady(t);
    const { paths } = fixture;
  ...
  return { ...fixture, state: loadWeeklyStatus(fixture.dir, fixture.date) };
}

test("weekly publish skips terminal states once without calls or artifacts", async (t) => {
  for (const status of ["failed", "published"]) await t.test(status, async (t) => {
    const f = await makeTerminalFixture(t, status);
    const { paths, state: prior } = f;
...
test("weekly terminal skips bind persisted content to the meeting and attempt", async (t) => {
  for (const mutation of ["foreign-failure", "failure-attempt", "failure-json", "failure-markdown",
    "missing-failure-json", "symlinked-failure-json", "published-report", "published-generation", "published-run"]) {
    await t.test(mutation, async (t) => {
      const f = await makeTerminalFixture(t, mutation.startsWith("published-") ? "published" : "failed");
      const state = f.state;
      const failureJson = state.failureArtifact?.replace(/\.md$/, ".json");
      if (mutation === "foreign-failure") {
        const date = "2026-09-23";
        const paths = buildWeeklyPipelinePaths(f.dir, date);
        const other = createWeeklyAttempt({ outputDir: f.dir, meetingDate: date, reportDepth: 3,
          attemptId: OTHER_ATTEMPT_ID, now: () => "2026-09-23T06:05:00.000Z" });
        const recorded = recordWeeklyFailure({ paths, state: other, stage: "collect",
          error: new Error("other meeting failure"), artifacts: [], redmineWriteAttempted: false,
          serverState: "unchanged", retryCommand: "retry", now: () => "2026-09-23T06:06:00.000Z" });
        state.failureArtifact = recorded.markdownPath;
      }
      if (mutation === "failure-attempt") state.pipelineAttemptId = OTHER_ATTEMPT_ID;
      if (mutation === "failure-json") {
        const failure = JSON.parse(fs.readFileSync(failureJson));
        fs.writeFileSync(failureJson, JSON.stringify({ ...failure, meetingDate: "2026-09-23" }));
      }
      if (mutation === "failure-markdown") fs.appendFileSync(state.failureArtifact, "changed failure reason\n");
      if (["missing-failure-json", "symlinked-failure-json"].includes(mutation)) {
        fs.renameSync(failureJson, `${failureJson}.retained`);
        if (mutation === "symlinked-failure-json") fs.symlinkSync(`${failureJson}.retained`, failureJson);
      }
      if (mutation === "published-report") fs.appendFileSync(f.reportPath, "changed report\n");
      if (mutation === "published-generation") {
        fs.writeFileSync(f.generationStatePath, JSON.stringify({ ...f.generationState, attemptId: OTHER_ATTEMPT_ID }));
      }
      if (mutation === "published-run") {
        const runStatePath = path.join(f.generationState.runDir, "state.json");
        const runState = JSON.parse(fs.readFileSync(runStatePath));
        fs.writeFileSync(runStatePath, JSON.stringify({ ...runState, attemptId: OTHER_ATTEMPT_ID }));
      }
      fs.writeFileSync(f.paths.statusPath, JSON.stringify(state));
      const before = fs.readFileSync(f.paths.statusPath);
      const files = fs.readdirSync(f.dir, { recursive: true });
      t.mock.method(global, "fetch", () => assert.fail("terminal verification must remain local"));
      await assert.rejects(() => runWeeklyPublish(f.config, f.meetingDate, {
        runUpdate: async () => assert.fail("untrusted terminal evidence must not publish"),
      }), { code: "terminal_state_invalid" });
      assert.deepStrictEqual(fs.readFileSync(f.paths.statusPath), before);
      assert.deepStrictEqual(fs.readdirSync(f.dir, { recursive: true }), files);
    });
  }
});
...
async function setupWeeklyReady(t, presentationCandidates = [], reportDepth = 3) {
  ...
  createWeeklyAttempt({ outputDir: fixture.dir, meetingDate: date, reportDepth,
...
test('depth2 weekly publication verifies the Wiki and skips the same completed attempt', async t => {
  const notionKey = process.env.NOTION_API_KEY;
  delete process.env.NOTION_API_KEY;
  t.after(() => { if (notionKey !== undefined) process.env.NOTION_API_KEY = notionKey; });
  const f = await setupWeeklyReady(t, [], 2);
  let text = `${f.config.env.sectionHeader}\n- existing\n`;
  let version = 1;
  let puts = 0;
  t.mock.method(global, 'fetch', async (url, options = {}) => {
    assert.ok(String(url).startsWith('http://weekly.invalid/projects/p/wiki/'));
    if (options.method === 'PUT') { text = JSON.parse(options.body).wiki_page.text; version++; puts++; }
    return new Response(JSON.stringify({ wiki_page: { text, version, updated_on: '2026-08-26T06:45:00Z' } }));
  });
  Object.assign(f.config.env, { baseUrl: 'http://weekly.invalid', apiKey: 'fixture',
    projectId: 'p', wikiUrl: 'http://weekly.invalid/projects/p/wiki/weekly' });
  const result = await runWeeklyPublish(f.config, f.meetingDate);
  assert.equal(result.state.status, 'published');
  assert.equal(result.state.reportDepth, 2);
  assert.equal(puts, 1);
  const bytes = fs.readFileSync(f.paths.statusPath);
  const retry = await runWeeklyPublish(f.config, f.meetingDate);
  assert.equal(retry.skipped, true);
  assert.equal(puts, 1);
  assert.deepEqual(fs.readFileSync(f.paths.statusPath), bytes);
});

test('depth2 prepare binds its configured depth to newly generated READY evidence', async t => {
  const f = await setupWeeklyReady(t, [], 2);
  const config = { ...f.config, env: { ...f.config.env, ...makeWeeklyPrepareConfig(f.dir).env, reportDepth: 2 } };
  const result = await runWeeklyPrepare(config, f.meetingDate, {
    randomUUID: () => OTHER_ATTEMPT_ID,
    runCollect: async () => ({ snapshot: f.snapshot, snapshotPath: f.snapshotPath }),
    runGenerate: () => runGenerateV2(f.config, f.meetingDate),
  });
  assert.equal(result.state.status, 'ready');
  assert.equal(result.state.reportDepth, 2);
  const generation = JSON.parse(fs.readFileSync(result.state.generationStatePath));
  assert.equal(generation.reportDepth, 2);
  assert.equal(result.state.reportHash, sha256(fs.readFileSync(result.state.reportPath)));
});

test('a depth2 publish rejects a depth3 READY but can inspect its already-published terminal evidence', async t => {
  const f = await setupWeeklyReady(t);
  f.config.env.reportDepth = 2;
  await assert.rejects(runWeeklyPublish(f.config, f.meetingDate, {
    runUpdate: () => assert.fail('mismatched depth must not reach publication'),
  }), { code: 'ready_evidence_mismatch' });
  const old = await makeTerminalFixture(t, 'published');
  old.config.env.reportDepth = 2;
  assert.equal((await runWeeklyPublish(old.config, old.meetingDate)).skipped, true);
});
...
test("weekly publish profile enforces unattended publishing without changing generic modes", () => {
  ...
  for (const [field, value] of Object.entries({ autoApprove: false, reportDepth: 4,
...
test("weekly publish CLI resolves terminal skips without credentials or new artifacts", async (t) => {
  const f = await makeTerminalFixture(t, "published");
  f.config.env.apiKey = "";
  const result = await runWeeklyCli(f, 'global.fetch = () => { throw new Error("terminal skip must not use network"); };');
  assert.strictEqual(result.status, 0, result.stderr);
  assert.strictEqual(result.stdout.split("\n").filter((line) => line.includes("[weekly][SKIP]")).length, 1);
  assert.deepStrictEqual(fs.readdirSync(f.paths.meetingDir), ["status.json"]);
});
...
test("weekly selected-note publication requires complete references and retains uncertain issue writes", async (t) => {
  for (const failureAt of ["lookup", "notion", "post-http", "post-lost", "post-id", "reuse-id", "second-lookup",
    ...
    await t.test(failureAt, async (t) => {
      ...
      const f = await setupWeeklyReady(t, notes, failureAt === "none" ? 2 : 3);
...
test("weekly detail overflow records its cause and rejected artifact without reaching READY", async (t) => {
  const f = setupSnapshot(t, {
    rawContent: ['#### <span style="color:blue">조현우</span>', '- PIM', '  - Application',
      ...Array.from({ length: 10 }, (_, i) => `    - [Notion] 상세 항목 ${i + 1}\n      ↳ 원인: 경계 불일치`), ''].join('\n'),
    autoContent: { "{{APP}}": "- updated" },
    categories: { app: { parent: "PIM", label: "Application", templateKey: "APP" } },
  });
  Object.assign(f.config.env, makeWeeklyPrepareConfig(f.dir).env);
  f.config.env.codexCli = makeFakeClaude(f, '{}');
  const logs = [];
  ...
  const state = loadWeeklyStatus(f.dir, '2026-08-26');
  const failure = failureFor(f.dir, '2026-08-26');
  assert.strictEqual(state.status, 'failed');
  assert.strictEqual(failure.primaryIssueCode, 'source_selection_detail_missing');
  assert.strictEqual(failure.issueCounts.source_selection_detail_missing, 1);
  assert.match(failure.representativeIssues[0].message, /R0010.*상세 항목 10/);
  assert.strictEqual(failure.redmineWriteAttempted, false);
  assert.strictEqual(failure.serverState, 'unchanged');
  assert.match(fs.readFileSync(state.failureArtifact, 'utf8'), /source_selection_detail_missing/);
  assert.match(logs.join('\n'), /\[weekly\]\[FAIL\].*source_selection_detail_missing/);
  const rejected = failure.artifacts.find(artifact => /report\.rejected\.001\.md$/.test(artifact.path));
  assert.ok(rejected);
  assert.strictEqual(rejected.sha256, sha256(fs.readFileSync(rejected.path)));
  assert.match(fs.readFileSync(rejected.path, 'utf8'), /↳ 원인: 경계 불일치/);
  assert.strictEqual(fs.existsSync(f.reportPath), false);
  let publishCalls = 0;
  await runWeeklyPublish(f.config, f.meetingDate, {runUpdate: async () => { publishCalls++; }});
