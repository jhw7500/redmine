...
test("run storage prevents overwriting the immutable AI draft and preserves every validation revision", (t) => {
  ...
  assert.strictEqual(fs.readFileSync(first.validationPath, "utf8").includes('"FAIL"'), true);
  assert.deepStrictEqual(JSON.parse(fs.readFileSync(paths.statePath, "utf8")).latestValidationPath, "validation.002.json");
});
