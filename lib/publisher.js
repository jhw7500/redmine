...
## 버그 검출 결과 보존 (최우선)
${guidance.preserveBugDetection || "테스트 프로젝트의 버그 검출 결과는 반드시 별도 라인으로 보존."}

## 버그 수정 보존
${guidance.preserveBugFixes || "실제 버그 수정은 기능 추가와 합치지 말 것."}

## 카테고리당 항목 수 상한
- 서브카테고리당 최대 ${maxPerSub}개 (초과 시 상위-하위 계층 그룹핑으로 묶어라)
- ETC 프로젝트당 최대 ${maxPerEtc}개
- **단, 버그 검출/해결 결과는 상한 계산에서 제외하고 모두 보존**`;
}
...
- 타입 그룹(구현/수정/리팩토링 등)이 하나뿐이면 타입 라벨 없이 바로 나열
- Test 카테고리 안에 독립 프로젝트(pim-check 등)가 있으면 프로젝트명을 하위 섹션으로 분리
