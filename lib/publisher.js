...
async function aiSummarize(rawContent, config, meetingDate) {
  ...
  const asOfFallback = meetingDate
...
- **테스트 결과 표기**: 원문의 \`PASS N/M\`은 N건 PASS·M건 FAIL 의미다. 출력은 \`N건 PASS, 실패 M건\` 또는 전체가 명확할 때만 \`N/T PASS\`로 쓴다. \`N/M PASS\`로 순서를 바꾸지 않는다.
...
async function generate(config, meetingDate, autoContent, noteRefs = [], options = {}) {
  ...
  const summarized = await aiSummarize(rawSection, config, meetingDate);
  const generatedSection = summarized ? sanitizeAiSection(summarized, config) : rawSection;
