...
async function generate(config, meetingDate, autoContent, noteRefs = []) {
  ...
  if (notesBlock) {
    ...
    if (/\n\*작성:/.test(generatedSection)) {
      // 함수형 replacer 사용 — 문자열 replacer의 $1/$&/$` 특수패턴 해석 방지
      // (notesBlock에 Notion 제목이 들어가 제목의 $ 시퀀스가 오치환될 수 있음).
      sectionWithNotes = generatedSection.replace(/(\n\*작성:[\s\S]*)$/, (_, footer) => `\n\n${notesBlock}${footer}`);
    } else {
      sectionWithNotes = generatedSection.trimEnd() + "\n\n" + notesBlock + "\n";
    }
  }
