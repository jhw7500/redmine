...
function buildRemoteEvidence(pageData, config) {
  if (!pageData || !pageData.wiki_page || typeof pageData.wiki_page.text !== "string") {
    throw new Error("Unexpected response: missing wiki_page.text during publish verification");
  }
  const section = extractSection(pageData.wiki_page.text, config);
  if (!section) {
    throw new Error("Could not find the target section during publish verification.");
  }
  return {
    section,
...
async function update(config, meetingDate, options = {}) {
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    try {
      ...
      if (!putSucceeded && /HTTP 409/.test(err.message) && attempt < MAX_PUT_RETRIES) {
