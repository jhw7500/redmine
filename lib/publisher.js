...
- 내용은 각 canonical heading 아래에서 압축·그룹화하고, 모든 N marker를 하위 bullet에 보존한다.`
    : "";
...
function buildSourceCoverageGuidance(catalog, options = {}) {
  ...
  return `## Source coverage marker — 출력에서 반드시 보존
- 원본에 포함된 모든 source marker를 해당 section heading 또는 대표 bullet과 함께 그대로 보존한다.
- 여러 항목을 한 bullet로 묶어도 각 source marker를 모두 남기며, ID를 바꾸거나 다른 category로 옮기지 않는다.
- source coverage marker는 생략 금지이며 해설하거나 일반 문자열로 바꾸지 않는다.
- 사용하지 않는 사실과 fact reference만 문장과 함께 생략할 수 있다. 이 규칙은 source marker에 적용하지 않는다.`;
}
