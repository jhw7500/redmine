...
function appendValidationRevision(paths, expectedAttemptId, validation) {
  ...
  writeImmutableArtifact(validationPath, JSON.stringify(validation, null, 2) + "\n");
  const next = updateRunState(paths, expectedAttemptId, {
    validationRevision: revision,
    latestValidationPath: filename,
  });
  return { revision, validationPath, state: next };
}
...
function acquirePromotionLock(generationStatePath) {
  ...
  try {
    ...
    if (error && error.code === "EEXIST") {
      throw new Error(`promotion is locked: ${generationStatePath}`);
    }
    throw error;
  }
  return () => {
...
function promoteRunReport({
  ...
  const releasePromotionLock = acquirePromotionLock(generationStatePath);
  try {
