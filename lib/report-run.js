...
const LOCK_HELPER_PATH = path.resolve(__dirname, "../scripts/hold-flock.sh");
...
function makeRunPaths(runDir) {
  return {
    ...
    coverageCatalogPath: path.join(runDir, "source-coverage.json"),
...
function loadResolvedReportRun(paths, attemptId) {
  ...
  let coverageCatalog = null;
  if (Object.prototype.hasOwnProperty.call(state, "sourceCoverageMode")) {
    coverageCatalog = readJson(paths.coverageCatalogPath, "coverage catalog");
    assertCoverageCatalogOwned(state, coverageCatalog);
  }
  return { paths, state, catalog, coverageCatalog };
}
...
function assertCoverageCatalogOwned(state, coverageCatalog) {
  if (!coverageCatalog || !state.coverageCatalogHash || !coverageCatalog.coverageCatalogHash) {
    throw new Error("coverage catalog hash is required");
  }
  if (state.coverageCatalogHash !== coverageCatalog.coverageCatalogHash) {
    throw new Error("coverage catalog hash mismatch");
  }
  const computedCoverageCatalogHash = hashObject({
  ...
  if (computedCoverageCatalogHash !== coverageCatalog.coverageCatalogHash) {
    throw new Error("coverage catalog hash mismatch");
  }
}

function assertRunInputs(state, snapshot, catalog, expected, coverageCatalog = null) {
...
function stopLockHolder(holder, lockPath, statusPath) {
  if (holder && holder.pid) holder.kill("SIGTERM");
  const status = waitForLockStatus(statusPath, new Set(["released"]));
...
function acquireFileLock(lockPath, lockError) {
  ...
  const statusPath = path.join(
    ...
    `.${path.basename(lockPath)}.status-${process.pid}-${crypto.randomUUID()}`
  );
  const holder = spawn("bash", [LOCK_HELPER_PATH, lockPath, statusPath], {
  ...
  if (status !== "locked") {
    ...
    fs.rmSync(statusPath, { force: true });
    if (status === "busy") throw lockError();
    throw new Error(`file lock helper timed out: ${lockPath}`);
  }
  ...
  return () => {
    if (released) return;
    released = true;
    stopLockHolder(holder, lockPath, statusPath);
  };
}
...
function assertSynchronousLockResult(result) {
  if (result && typeof result.then === "function") {
    ...
    const error = new Error("file-lock callbacks must be synchronous");
    error.code = "ASYNC_FILE_LOCK_CALLBACK";
    throw error;
  }
  return result;
}
...
function withGenerationStateLock(generationStatePath, operation) {
  ...
  try {
    return assertSynchronousLockResult(operation());
  } finally {
    release();
  }
}
