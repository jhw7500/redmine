...
const TERMINAL_STATUSES = new Set(["ai_failed", "complete", "validation_failed"]);
const LOCK_HANDSHAKE_TIMEOUT_MS = 5 * 1000;
const LOCK_WAIT_BUFFER = new Int32Array(new SharedArrayBuffer(4));
const DIRECTORY_OPEN_FLAGS = fs.constants.O_RDONLY
  | fs.constants.O_DIRECTORY
  | fs.constants.O_NOFOLLOW;
const STATE_OPEN_FLAGS = fs.constants.O_RDONLY
  | fs.constants.O_NOFOLLOW
  | fs.constants.O_NONBLOCK;
const LOCK_PROBE_OPEN_FLAGS = fs.constants.O_RDWR
...
function readRunState(runDir, summary) {
  let stateDescriptor;
  try {
    stateDescriptor = fs.openSync(
      path.join(runDir, "state.json"),
      STATE_OPEN_FLAGS
    );
    if (!fs.fstatSync(stateDescriptor).isFile()) {
      incrementReason(summary, "invalid_state");
      return null;
    }
    return JSON.parse(fs.readFileSync(stateDescriptor, "utf8"));
  } catch (error) {
    incrementReason(summary, "invalid_state");
    return null;
  } finally {
    if (stateDescriptor !== undefined) fs.closeSync(stateDescriptor);
  }
}
...
function validationLockBusy(runDir, summary) {
  ...
  let lockDescriptor;
  try {
    lockDescriptor = fs.openSync(lockPath, LOCK_PROBE_OPEN_FLAGS);
    if (!fs.fstatSync(lockDescriptor).isFile()) {
  ...
  try {
    const probe = spawnSync("flock", ["-n", "3"], {
