...
function restoreUnmarkedIdentifierReferences(content, catalog, options = {}) {
  const source = String(content);
  const parsed = parseAnnotatedDraft(source);
  const markedSpans = [
    ...parsed.claims.map((claim) => ({
  ...
  const recoverable = extractSchemaV2FactOccurrences(source, { markedSpans })
    ...
    .map((occurrence) => {
      const outputPath = options.knownPaths === undefined
        ? null
        : sourceSectionPathAt(source, occurrence.start, options.knownPaths);
      const candidates = (factsBySurface.get(occurrence.raw) || []).filter((fact) =>
        options.knownPaths === undefined || sameSectionPath(fact.sourceSectionPath, outputPath)
      );
      return {
        occurrence,
        fact: selectIdentifierFact(candidates, lineAt(source, occurrence.start), occurrence.raw),
      };
    })
    .filter(({ fact }) => fact);
