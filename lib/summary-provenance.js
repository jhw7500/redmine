...
function bindSummaryProvenance(autoContent, rawContent, spans, repos = {}, emittedBlocks = {}) {
  ...
  for (const { templateKey } of spans) {
    ...
    const emittedBlock = Object.hasOwn(emittedBlocks, templateKey)
      ? emittedBlocks[templateKey] : undefined;
...
function summarySourceLines(snapshot) {
  ...
  if (Object.hasOwn(details, "blocks") && (!details.blocks || typeof details.blocks !== "object"
  ...
  for (const span of details.spans) {
    ...
    if (!blocks.has(span.templateKey)) {
      const emittedBlock = details.blocks && Object.hasOwn(details.blocks, span.templateKey)
        ? details.blocks[span.templateKey] : undefined;
