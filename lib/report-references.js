...
function invalid() {
  ...
  throw Object.assign(new Error('Invalid report reference; check binding, URL and team audience'),
...
function validateBinding(entry) {
  ...
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)
    ...
    || !UUID.test(entry.sourceId.slice(7)) || !UUID.test(entry.referencePageId)
    || entry.audience !== 'team' || !shortText(entry.label, 32)
...
async function resolveReportReferences(items, config, fetchPage) {
  ...
  for (const item of items) {
    const matched = bindings.get(item.sourceId);
    ...
    for (const entry of matched) {
      ...
      if (!page || page.id !== entry.referencePageId || page.archived || page.in_trash
      ...
      references.push({ referencePageId: entry.referencePageId, label: entry.label, url, audience: 'team',
...
function referenceLines(item, indent) {
  ...
  const links = item.reportReferences.map(reference => {
    if (!reference || reference.audience !== 'team' || !shortText(reference.label, 32)
      || (reference.version !== undefined && !shortText(reference.version, 48))) invalid();
    const label = reference.label + (reference.version ? ` (${reference.version})` : '');
...
module.exports = { resolveReportReferences, referenceLines, safeReferenceUrl, assertReferenceGenerationMode };
