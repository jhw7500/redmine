...
const { assertWikiRevisionEvidence } = require("./publisher");
...
function assertWeeklyStateSchema(paths, state, { allowUnavailableEvidence = false } = {}) {
  ...
  if (state.status === "published") {
    ...
    assertWikiRevisionEvidence(state.published);
  }
...
function aggregateValidationIssues(validation) {
  ...
  for (const issue of normalized) {
    if (issue.severity === "error") blockingCounts.set(issue.code, (blockingCounts.get(issue.code) || 0) + 1);
  }
  const codeOrder = [...counts].sort((left, right) => (
    (blockingCounts.get(right[0]) || 0) - (blockingCounts.get(left[0]) || 0)
...
function assertWeeklyReadyEvidence(paths, state, config, meetingDate) {
  ...
  try {
    assertWeeklyStateSchema(paths, state);
    if (![2, 3].includes(state.reportDepth) || state.reportDepth !== config.env.reportDepth) {
      throw new Error("weekly READY depth must match the configured depth");
    }
    ...
    if (snapshot.status !== "sealed" || snapshot.meetingDate !== state.meetingDate
      ...
      || generation.reportDepth !== state.reportDepth) {
...
function assertWeeklyTerminalEvidence(paths, state, config, meetingDate) {
  assertWeeklyStateSchema(paths, state);
  if (![2, 3].includes(state.reportDepth)) throw new Error("weekly terminal depth must be 2 or 3");
  if (state.status === "published") {
    assertWeeklyReadyEvidence(paths, state, { ...config, env: { ...config.env,
      reportDepth: state.reportDepth, snapshotPath: state.snapshotPath, outputPath: state.reportPath } }, meetingDate);
  } else {
    const markdownPath = state.failureArtifact;
    if (path.dirname(markdownPath) !== paths.failuresDir || !markdownPath.endsWith(".md")) {
      throw new Error("weekly failure artifact must belong to this meeting");
    }
    const jsonPath = markdownPath.replace(/\.md$/, ".json");
    assertOwnedEvidenceFile(paths, jsonPath, "weekly failure JSON");
    const json = fs.readFileSync(jsonPath, "utf8");
    const failure = JSON.parse(json);
    if (!failure || failure.schemaVersion !== 1 || failure.meetingDate !== state.meetingDate
      || (failure.reportDepth === undefined ? 3 : failure.reportDepth) !== state.reportDepth
      || failure.pipelineAttemptId !== state.pipelineAttemptId || failure.stage !== state.stage
      || path.basename(markdownPath) !== `${weeklyFailureTimestamp(failure.occurredAt)}-${state.stage}-${state.pipelineAttemptId}.md`) {
      throw new Error("weekly failure evidence ownership mismatch");
    }
    if (fs.readFileSync(markdownPath, "utf8") !== renderWeeklyFailureMarkdown(failure)
      || fs.readFileSync(jsonPath, "utf8") !== json) {
      throw new Error("weekly failure evidence content mismatch");
    }
  }
  if (JSON.stringify(readWeeklyStatus(paths)) !== JSON.stringify(state)) {
    throw new Error("weekly terminal state changed during verification");
  }
}
...
async function runWeeklyPublish(config, meetingDate, dependencies = {}) {
  ...
  if (state && ["failed", "published"].includes(state.status)) {
    try {
      assertWeeklyTerminalEvidence(paths, state, config, meetingDate);
    } catch (cause) {
      // Invalid ownership cannot authorize a new failure record or overwrite.
      // Preserve the suspect state and make the wrapper alert instead of silently skipping.
      const error = weeklyPublishError("terminal_state_invalid", "weekly terminal state evidence is invalid; inspect status.json", cause);
      console.error(formatWeeklyFailureLog({ stage: "publish", code: error.code, message: error.message }, paths.statusPath));
      throw error;
    }
  ...
  if (!state) state = createWeeklyAttempt({ outputDir: config.env.outputDir, meetingDate: date,
    reportDepth: config.env.reportDepth, attemptId: (dependencies.randomUUID || crypto.randomUUID)(), now });
  ...
  try {
    ...
    const verifyRemote = (publication) => {
      ...
      assertWikiRevisionEvidence(publication.remote);
      return current;
    };
...
function writeDurableImmutableFailure(filePath, content) {
  const directory = path.dirname(filePath);
  ...
  try {
    ...
    try {
      ...
      if (!stats.isFile() || stats.uid !== process.getuid() || (stats.mode & 0o777) !== 0o600) {
        throw new Error("weekly failure file must be an owned private regular file");
      }
    ...
    try {
      ...
      if (error.code === "EEXIST") throw new Error(`immutable artifact already exists: ${filePath}`);
      throw error;
    }
  } finally {
...
function recordWeeklyFailure({
  ...
  writeDurableImmutableFailure(jsonPath, `${JSON.stringify(failure, null, 2)}\n`);
  assertNotSymlink(paths.failuresDir, "weekly failures directory");
  writeDurableImmutableFailure(markdownPath, renderWeeklyFailureMarkdown(failure));
  const directoryDescriptor = fs.openSync(paths.failuresDir,
    fs.constants.O_RDONLY | fs.constants.O_DIRECTORY | fs.constants.O_NOFOLLOW);
  try {
    fs.fsyncSync(directoryDescriptor);
  } finally {
