...
// Notion 페이지의 'report' select 필드 → 보고서 카테고리 매핑.
// Notion DB에 사용자가 수동 지정한 보고 분류를 우선 사용한다.
// select 옵션은 hyphen-case 사용자 친화 형태이고, 코드에서 catToTemplate key로 변환.
const REPORT_SELECT_TO_CATEGORY = {
  "pim-app": "pimApp",
...
// 'none' / 'private' / 'note' 등은 보고 제외 표시.
const REPORT_SKIP_VALUES = new Set(["none", "private", "note", "no-report", "skip"]);

// 반환: { skip: true } | { category: 'wlanBsp' } | null (필드 없음)
function extractReportField(page) {
  const prop = page.properties?.report;
  if (!prop || prop.type !== "select" || !prop.select) return null;
  const name = (prop.select.name || "").trim().toLowerCase();
  if (!name) return null;
  if (REPORT_SKIP_VALUES.has(name)) return { skip: true };
  const cat = REPORT_SELECT_TO_CATEGORY[name];
...
async function collectNotionItems(config, startDate, endDate) {
  ...
  for (const page of projectPages) {
    ...
    const reportField = extractReportField(page);
  ...
  for (const page of kbPages) {
    ...
    const category = reportField?.category
      || mapCategory(title + " " + (summary || ""), projectMapping)
      || undefined;
    items.push({
      source: "notion",
      title,
