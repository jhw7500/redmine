...
async function readNotionJson(response) {
  if (!response.ok) {
    ...
    throw Object.assign(new Error(`[notion-api] HTTP ${response.status}: Notion request failed`), {
      code: "NOTION_HTTP_FAILED", status: response.status,
    });
  }
  try {
    return await response.json();
  } catch {
    throw Object.assign(new Error("[notion-api] Failed to parse Notion JSON response"), {
      code: "NOTION_JSON_INVALID", status: response.status,
    });
  }
}

async function fetchNotionPage(apiKey, pageId, fetchImpl = fetch) {
  if (!apiKey || !/^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/i.test(pageId)) {
    throw Object.assign(new Error('Notion page lookup requires a key and page UUID'), { code: 'NOTION_PAGE_INVALID' });
  }
  return readNotionJson(await fetchImpl(`https://api.notion.com/v1/pages/${pageId}`, { headers: headers(apiKey) }));
}
...
function extractDate(page, propName) {
  const prop = (page.properties || {})[propName];
  if (!prop || prop.type !== "date" || !prop.date) return "";
  return prop.date.start || "";
}

function formatDateInTimeZone(input, timeZone = "Asia/Seoul") {
...
async function fetchAllProjectsTitleMap(apiKey) {
  ...
  do {
    ...
    const data = await readNotionJson(res);
...
async function queryDatabase(apiKey, dbDef, startDate, endDate, fetchImpl = fetch) {
  let filter;
  ...
  if (dbDef.dateFilterType === "created_time") {
    ...
    // date 타입은 시각이 없으므로 KST calendar day 기준이다. 마지막 날도 보수적으로 전부 포함한다.
    const startDay = formatDateInTimeZone(startDate);
    const endDay = formatDateInTimeZone(endDate);
    filter = {
      and: [
        { property: dbDef.dateProperty, date: { on_or_after: startDay } },
        { property: dbDef.dateProperty, date: { on_or_before: endDay } },
      ],
    };
  }
  ...
  do {
    ...
    const res = await fetchImpl(`https://api.notion.com/v1/databases/${dbDef.id}/query`, {
      method: "POST",
      headers: headers(apiKey),
...
function pageWithinRange(page, startDate, endDate) {
...
function baseNotionItem(page, sourceDb) {
  return {
    source: "notion",
    sourceDb,
...
function retainPresentationTaggedPage(reportField, tags) {
  return Boolean(reportField?.skip) && Array.isArray(tags) && tags.includes("발표노트");
}

// --- 메인 수집 ---

async function collectNotionItems(
  config,
  startDate,
  endDate,
  ...
  for (const page of projectPages) {
    if (!pageWithinRange(page, startDate, endDate)) continue;
    ...
    const category = reportField?.category
      ...
      || undefined;
    items.push({
      ...baseNotionItem(page, "projects"),
      title,
      summary: desc || undefined,
      category,
      report: extractSelect(page, "report") || undefined,
  ...
  for (const page of decisions) {
    ...
    const category = reportField?.category
      ...
      || undefined;
    items.push({
      ...baseNotionItem(page, "decisionLog"),
      timestamp: null,
      date: extractDate(page, DATABASES.decisionLog.dateProperty).slice(0, 10),
      timePrecision: "day",
      title,
  ...
  for (const page of kbPages) {
    ...
    const taggedForPresentation = tags.includes("발표노트");
    if (!title || (isExcluded(title, excludePatterns) && !taggedForPresentation)) continue;
    const summary = extractRichText(page, "summary");
    const projectText = resolveProjectText(page, projectsTitleMap);
    const reportField = extractReportField(page, reportMapping, reportSkipSet);
    if (reportField?.skip && !retainPresentationTaggedPage(reportField, tags)) {
      kbSkipped++;
      continue;
    }
    const category = reportField?.category
      ...
      || undefined;
    items.push({
      ...baseNotionItem(page, "knowledgeBase"),
      title,
      summary: summary || undefined,
      category,
      report: extractSelect(page, "report") || undefined,
      tags,
      reportExcluded: Boolean(reportField?.skip),
    });
  }
  ...
  for (const page of refPages) {
    ...
    const category = reportField?.category
      ...
      || undefined;
    items.push({
      ...baseNotionItem(page, "references"),
      title,
      summary: summary || undefined,
      category,
      report: extractSelect(page, "report") || undefined,
