...
function parseSourceSelection(text, catalog, leaderHighlight) {
  let value;
...
function renderSourceSelection(catalog, selection, options = {}) {
  const valid = parseSourceSelection(JSON.stringify(selection), catalog, options.leaderHighlight);
  const byId = new Map(catalog.records.map(record => [record.id, record]));
...
function buildSelectionPrompt(catalog, config) {
  ...
  const highlightRule = leaderHighlight.enabled
    ? `중요한 PIM/Wireless Lan 항목만 highlight=true. ${leaderHighlight.maxLines > 0
      ? `모든 section을 합쳐 highlight=true 항목은 최대 ${leaderHighlight.maxLines}개.` : '별도 강조 개수 상한 없음.'}\n${leaderHighlight.guidance || ''}`
...
depth=${config.env.reportDepth}: section별 가능한 1~2개 핵심 항목.
${highlightRule}
