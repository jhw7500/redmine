...
// claude 계열은 JSON을 마크다운 코드 펜스로 감싸 돌려준다. 펜스는 형식이지 내용이 아니므로
// 정확히 감싸진 경우만 벗겨 읽는다 — 그 밖의 관용은 두지 않는다.
...
function parseSourceSelection(text, catalog, leaderHighlight, reportDepth) {
  let value;
  try { value = JSON.parse(stripCodeFence(text)); } catch { throw selectionError("Selection must be one JSON object"); }
