...
// HTTP 본문과 JSON 파싱 예외에는 비공개 내용이 포함될 수 있다. 상태와 안정된 코드만 전달한다.
function requestError(code, status, message) {
  return Object.assign(new Error(`HTTP ${status}: ${message}`), { code, status });
}

async function checkedResponse(url, options, allowMissingCompletedTag = false) {
  const res = await fetch(url, options || {});
  if (!res.ok) {
    let code = "PRESENTATION_HTTP_FAILED";
    if (allowMissingCompletedTag && res.status === 400) {
      try {
        const data = JSON.parse(await res.text());
        if (data.code === "validation_error"
          && typeof data.message === "string"
          && data.message.startsWith(`multi_select option "${COMPLETED_TAG}" not found for property "tags".`)) {
          code = "PRESENTATION_TAG_MISSING";
        }
      } catch { /* Unrecognized bodies remain ordinary HTTP failures. */ }
    }
    if (res.body) await res.body.cancel().catch(() => {});
    throw requestError(code, res.status, "Presentation-note request failed");
  }
  return res;
}

// Redmine writes may return 204 No Content; do not try to parse a success body.
async function fetchNoContent(url, options) {
  await checkedResponse(url, options);
}

async function fetchJson(url, options, allowMissingCompletedTag = false) {
  const res = await checkedResponse(url, options, allowMissingCompletedTag);
  try {
    return await res.json();
  } catch {
    throw requestError("PRESENTATION_JSON_INVALID", res.status, "Invalid presentation-note JSON response");
  }
}
  ...
  for (;;) {
    ...
    for (const it of issues) {
      if (hasMarker(it.description || "", pageId)) {
        if (!Number.isSafeInteger(it.id) || it.id <= 0) {
          throw Object.assign(new Error("Existing presentation-note issue has an invalid ID"), {
...
class IssueFieldMismatchError extends Error {
  constructor({ issueId, mismatches }) {
    super(`createIssue: ` + mismatches
      .map((m) => `${m.field} 미반영 (요청 ${m.requested}, 실제 ${m.actual}) #${issueId}. ${m.hint}.`)
      .join(" "));
    this.name = "IssueFieldMismatchError";
    this.issueId = issueId;
    this.mismatches = mismatches;
    this.fields = mismatches.map((m) => m.field);
  }
}
...
async function createIssue(env, meta, note, md, dateStr) {
  ...
  if (!data || !data.issue || !Number.isSafeInteger(data.issue.id) || data.issue.id <= 0) {
    throw Object.assign(new Error("createIssue: 예상치 못한 응답 — issue.id가 없다."), {
      code: "PRESENTATION_RESPONSE_INVALID",
    });
  }
  // 트래커는 활성 목록에서 고르는 사전 선택으로 막지만(위 resolveMeta), status는 사후 검증만 가능하다.
  // Redmine이 허용되지 않은 status_id를 오류 없이 기본값으로 폴백하기 때문이다.
  // 첫 불일치에서 바로 던지면 뒤 필드를 검사하지 못한 채 호출부가 "이상 없음"으로
  // 기록하게 된다. 전부 검사한 뒤 한 번에 던져야 플래그가 사실과 일치한다.
  const mismatches = [];
  const gotStatus = data.issue.status && data.issue.status.id;
  if (gotStatus !== meta.statusId) {
    mismatches.push({
      field: "status_id", requested: meta.statusId, actual: gotStatus,
      hint: gotStatus === undefined
  ...
  if (gotAssignee !== meta.assigneeId) {
    mismatches.push({
      field: "assigned_to_id", requested: meta.assigneeId, actual: gotAssignee,
      hint: "대상 사용자가 이 프로젝트의 할당 가능한 멤버인지 확인 필요",
    });
  }
  if (mismatches.length) {
    throw new IssueFieldMismatchError({ issueId: data.issue.id, mismatches });
  }
...
async function queryCompletedNotes(env) {
  ...
  try {
    ...
    if (err.code === "PRESENTATION_TAG_MISSING") {
...
async function publishNotes(env, notes, opts = {}) {
  const meta = await resolveMeta(env);
  const refs = [];
  let issueWriteAttempted = false;
  const requireNote = (error) => {
    if (!opts.requireAll) return;
    if (issueWriteAttempted) error.serverState = "written_unverified";
    throw error;
  };
  for (const note of notes) {
    let md;
    ...
    if (opts.assertReady) {
      try { await opts.assertReady(); }
      catch (err) { requireNote(err); throw err; }
    }
    try {
      issueWriteAttempted = true;
      ...
      if (err instanceof IssueFieldMismatchError) {
        ...
        console.warn(
          `[issue] created #${err.issueId} 불일치 [${err.fields.join(", ")}]: ` +
          `${buildSubject(note)} — 보고서에는 포함하되 Redmine에서 직접 교정하라.`
        );
        refs.push({
          id: err.issueId, title: note.title, notionUrl: note.notionUrl, reused: false,
          statusMismatch: err.fields.includes("status_id"),
          assigneeMismatch: err.fields.includes("assigned_to_id"),
        });
        continue;
      }
      requireNote(err);
